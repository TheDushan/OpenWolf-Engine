...
sfxHandle_t idAudioOpenALSystemLocal::buf_find_free( void )
{
    ...
    // Shit...
    trap_Error( ERR_FATAL, "idAudioOpenALSystemLocal::buf_find_free: No free sound handles" );
    return -1;
}
...
=================
*/
sfxHandle_t idAudioOpenALSystemLocal::RegisterSound( pointer sample, bool compressed )
{
...
void idAudioOpenALSystemLocal::buf_load( sfxHandle_t sfx )
{
    ...
    if( error != AL_NO_ERROR )
    {
        ...
        trap_Hunk_FreeTempMemory( data );
        trap_Printf( PRINT_ALL, "Can't fill sound buffer for %s - %s", knownSfx[sfx].filename, errormsg( error ) );
        return;
    }
...
sint idAudioOpenALSystemLocal::duration( sfxHandle_t sfx )
{
    if( sfx < 0 || sfx >= numSfx )
    {
        trap_Printf( PRINT_ALL, "ERROR: idAudioOpenALSystemLocal::SoundDuration: handle %i out of range\n", sfx );
        return 0;
    }
...
void idAudioOpenALSystemLocal::buf_unload( sfxHandle_t sfx )
{
    ...
    if( ( error = qalGetError() ) != AL_NO_ERROR )
    {
        trap_Printf( PRINT_ALL, "Can't delete sound buffer for %s", knownSfx[sfx].filename );
    }
...
sint idAudioOpenALSystemLocal::GetSoundLength( sfxHandle_t sfxHandle )
{
    ...
    return ( sint )( ( ( float32 )knownSfx[sfxHandle].info.samples / ( float32 )knownSfx[sfxHandle].info.rate ) * 1000.0f );
}
