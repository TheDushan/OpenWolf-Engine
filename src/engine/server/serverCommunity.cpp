...
===============
*/
sint idServerCommunityServer::Login( valueType* userinfo, user_t** user_ret )
{
    valueType* password = nullptr, * username = nullptr, * userpassword = nullptr, buffer[512], *use_private_slot;
    netadr_t from;
    user_t* user;
    ...
    networkSystem->StringToAdr( Info_ValueForKey( userinfo, "ip" ), &from, NA_UNSPEC );
    ...
    if( use_private_slot != nullptr && user != nullptr && ( user->type == CS_OWNER || user->type == CS_ADMIN ) )
    {
...
sint idServerCommunityServer::alreadyLogin( valueType* username )
{
    ...
    {
        if( svs.clients[i].state >= CS_ACTIVE && svs.clients[i].cs_user != nullptr )
        {
...
sint idServerCommunityServer::checkMatchInvited( user_t* user )
{
    ...
    for( i = 0; i < MATCH_MAX_USERS && ::strlen( match_referees[i] ) != 0 && ::strcmp( user->name, match_referees[i] ) != 0; i++ );
    ...
    for( i = 0; i < MATCH_MAX_USERS && ::strlen( match_users[i] ) != 0 && ::strcmp( user->name, match_users[i] ) != 0; i++ );
    ...
    for( user_clan = user->clans; user_clan != nullptr; user_clan = user_clan->next )
    {
        for( i = 0; i < MATCH_MAX_CLANS && ::strlen( match_clans[i] ) != 0 && ::strcmp( user_clan->clan->name, match_clans[i] ) != 0; i++ );
...
void idServerCommunityServer::LoadUserFile( void )
{
    ...
    if( hash_users != nullptr || hash_clans != nullptr || usersfile != nullptr )
    {
        Com_Printf( "YOU CANNOT RELOAD FILE UNTIL YOU RESET HASH TABLES AND FILE DESCRIPTORS!!!\n" );
        return;
    }
    
    hash_users = Com_CreateHashTable( Com_JenkinsHashKey, Com_StrCmp, Com_DestroyStringKey, destroyLongData, HASH_USER_SIZE );
    hash_clans = Com_CreateHashTable( Com_JenkinsHashKey, Com_StrCmp, Com_DestroyStringKey, destroyClanData, HASH_CLAN_SIZE );
...
valueType** idServerCommunityServer::fastParseLine( void* userline )
{
    ...
    if( reg[SV_COMMUNITY_REGDATA] == nullptr || ::strlen( reg[SV_COMMUNITY_REGDATA] ) > USER_NAME_SIZE )
    {
...
{
    sint32* dpos;
    valueType* username, * type, * tigerhash, * clan, userline[1024], buffer[512], client_user_pass[1024];
    user_t* user;
    user_clan_t* user_clan;
    clan_t* clan_p;
    ...
    else if( type[0] != 'A' && type[0] != 'M' && type[0] != 'O' && type[0] != 'B' )
    {
...
void idServerCommunityServer::destroyUserData( user_t* user )
{
    ...
    for( user_clan = ( user )->clans; user_clan != nullptr; user_clan = user_clan_next )
    {
...
===============
*/
void idServerCommunityServer::logString( sint client_num, valueType* area, valueType* message )
{
    static sint arch = -1;
    ...
    if( arch < 0 )
    {
        ...
        arch = ::open( fileSystem->GetFullGamePath( logFile->string ), O_WRONLY | O_APPEND );
...
void idServerCommunityServer::addMatchClan( valueType* clan_name )
{
    ...
    for( i = 0; i < MATCH_MAX_CLANS && ::strlen( match_clans[i] ) != 0 && ::strcmp( match_clans[i], clan_name ) != 0; i++ );
...
void idServerCommunityServer::addMatchUser( valueType* user_name )
{
    ...
    for( i = 0; i < MATCH_MAX_USERS && ::strlen( match_users[i] ) != 0 && ::strcmp( match_users[i], user_name ) != 0; i++ );
...
void idServerCommunityServer::addMatchReferee( valueType* user_name )
{
    ...
    for( i = 0; i < MATCH_MAX_USERS && ::strlen( match_referees[i] ) != 0 && ::strcmp( match_referees[i], user_name ) != 0; i++ );
...
void idServerCommunityServer::LoadBanFile( void )
{
    ...
    hash_bans = Com_CreateHashTable( Com_JenkinsHashKey, Com_StrCmp, Com_DestroyStringKey, destroyLongData, HASH_BAN_SIZE );
...
void idServerCommunityServer::BanUser( client_t* cl )
{
    ...
    *dpos = ::ftell( bansfile );
    ...
    {
        ::strncpy( banuser.name, Info_ValueForKey( cl->userinfo, "name" ), USER_NAME_SIZE );
        banuser.type = CS_UNKNOW;
    }
    
    banuser.until = 0x7FFFFFFF;
    
    ::sprintf( buffer, "%s:%12d:%c:%s\n", banuser.name, banuser.until, banuser.type, guid );
...
void idServerCommunityServer::showBanUsers( void )
{
    ...
    while( ( data = Com_HashIterationData( iter ) ) != nullptr )
    {
        ...
        if( banuser != nullptr )
        {
            Com_Printf( "%d: %s - %c Banned until %s", i++, banuser->name, banuser->type, ctime( ( const time_t* ) & ( banuser->until ) ) );
        }
    }
...
===============
*/
sint idServerCommunityServer::checkClientCommandPermitions( client_t* cl, pointer client_cmd )
{
    sint i;
    ...
    {
        serverMainSystem->SendServerCommand( cl, "chat \"^9YOU MUST BE MEMBER TO SEND %s COMMAND!\"", cmd );
        return CS_ERROR;
    }
...
sint idServerCommunityServer::StatsLoop( void )
{
    ...
    if( ( ( actual_update - last_update ) >= 1000 && community_stats.mapstatus == 2 ) || ( ( actual_update - last_update ) >= 3000 && community_stats.mapstatus != 2 ) )
    {
...
===============
*/
sint idServerCommunityServer::GetDescValue( valueType* line, valueType* desc, valueType* val )
{
    valueType* tmp;
    sint i;
...
void idServerCommunityServer::ProcessSplit( valueType** tokens )
{
    ...
    {
        ::strncpy( username, Info_ValueForKey( svs.clients[stats_client_num].userinfo, "cl_guid" ), 32 );
        username[32] = '\0';
        type = CS_GUID;
    }
    else
...
===============
*/
user_stats_t* idServerCommunityServer::GetPlayerStats( valueType* player_name )
{
    valueType username[USER_NAME_SIZE + 1] = {0};
    user_stats_t** paux;
...
===============
*/
sint idServerCommunityServer::ProcessStats( user_stats_t* user_stats, valueType** tokens )
{
    valueType val[512];
...
===============
*/
void idServerCommunityServer::ProcessWeapon( user_stats_t* user_stats, valueType* weapon, valueType** tokens )
{
    ...
    for( iweapon = 0; l_weapons[iweapon] != nullptr && ::strcmp( l_weapons[iweapon], weapon ) != 0; iweapon++ );
    ...
    ProcessGeneric( l_stats_weapons, &( tokens[3] ), user_stats->weapons[iweapon] );
}
...
===============
*/
void idServerCommunityServer::ProcessMELEE( user_stats_t* user_stats, valueType** tokens )
{
...
===============
*/
void idServerCommunityServer::ProcessMISC( user_stats_t* user_stats, valueType** tokens )
{
...
===============
*/
void idServerCommunityServer::ProcessGeneric( pointer* l_stats, valueType** tokens, sint* i_stats )
{
...
void idServerCommunityServer::SaveStatistics( void )
{
    ...
    for( pu_del = nullptr, pu_iter = community_stats.user_stats; pu_iter != nullptr; pu_del = pu_iter, pu_iter = pu_iter->next )
    {
        old_offset = offset;
        offset += SaveUserStatsOptimal( pu_iter, &( sv_community_last_match_s[offset] ) );
...
===============
*/
void idServerCommunityServer::SaveUserStats( user_stats_t* user_stats, FILE* fstats )
{
    sint i, j, offset = 0;
    ...
    offset += ::sprintf( &( fline[offset] ), "M|%s|%s", community_stats.game_id, user_stats->name );
    ...
    for( i = 0; i < WEAPONS; i++ )
    {
        ...
        {
            ::fprintf( fstats, "|%s:%d", weapons_stats_mnemonics[j], user_stats->weapons[i][j] );
    ...
    {
        ::fprintf( fstats, "|%s:%d", melee_stats_mnemonics[i], user_stats->melee[i] );
    ...
    {
        ::fprintf( fstats, "|%s:%d", explosions_stats_mnemonics[i], user_stats->explosions[i] );
...
===============
*/
sint idServerCommunityServer::SaveUserStatsOptimal( user_stats_t* user_stats, valueType* fline )
{
    pointer weapons_mnemonics[WEAPONS + 1] = { WEAPONS_MNEMONICS };
    pointer weapons_stats_mnemonics[WEAPON_STATS + 1] = { WEAPONS_STATS_MNEMONICS };
    pointer melee_stats_mnemonics[MELEE_STATS + 1] = { MELEE_STATS_MNEMONICS };
    ...
    offset += ::sprintf( &( fline[offset] ), "{\"ST\":\"P\",\"PLAYER\":{\"GENERAL\":{\"GID\":\"%s\",\"PTY\":\"%d\",\"NME\":\"%s\",\"GNM\":\"%s\",\"AVP\":\"%d\",\"TME\":\"%d\",\"GME\":\"%d\",\"LIN\":\"%d\",\"SRE\":\"%d\",\"TEM\":\"%d\"}",
                         community_stats.game_id, user_stats->type, user_stats->name, user_stats->game_name,
                         user_stats->avg_ping, user_stats->time / 1000, user_stats->game_time / 1000,
    ...
    for( i = 0; i < WEAPONS; i++ )
    {
        offset += PrintStat( &( fline[offset] ), WEAPON_STATS, weapons_mnemonics[i], user_stats->weapons[i], weapons_stats_mnemonics );
    ...
    oldoffset = offset;
    offset += PrintStat( &( fline[offset] ), MELEE_STATS, "M", user_stats->melee, melee_stats_mnemonics );
    if( oldoffset != offset )
    {
        fline[oldoffset] = ',';
    }
    ...
    oldoffset = offset;
    offset += PrintStat( &( fline[offset] ), MISC_STATS, "I", user_stats->misc, misc_stats_mnemonics );
    if( oldoffset != offset )
    {
        fline[oldoffset] = ',';
    }
...
===============
*/
sint idServerCommunityServer::PrintStat( valueType* out, sint count_stats, pointer section_mnemonic, const sint* stats, pointer* stats_mnemonics )
{
    sint boffset = 0;
    ...
    for( i = 0; i < count_stats; i++ )
    {
        if( stats[i] != 0 )
        {
            ...
            boffset += ::sprintf( &( buffer[boffset] ), "\"%s\":\"%d\"", stats_mnemonics[i], stats[i] );
        }
    }
...
void idServerCommunityServer::InitStatistics( void )
{
    ...
    sprintf( buffer, "{\"ST\":\"G\",\"GAME\":{\"PRT\":\"%d\",\"GID\":\"%s\",\"MAP\":\"%s\",\"STM\":\"%d\",\"GTY\":\"%d\",\"STS\":\"%d\"}}\n",
...
void idServerCommunityServer::PlayerServerInfo( sint clnum )
{
    ...
    if( svs.clients[clnum].state >= CS_ACTIVE )
    {
        ...
        {
            ::strncpy( username, Info_ValueForKey( svs.clients[clnum].userinfo, "cl_guid" ), 32 );
            username[32] = '\0';
            type = CS_GUID;
        }
        else
        ...
        {
            user_stats->type = type;
            ::strncpy( user_stats->game_name, svs.clients[stats_client_num].name, sizeof( user_stats->game_name ) );
...
void idServerCommunityServer::ServerInfo( void )
{
    ...
    for( i = 0; i < sv_maxclients->integer; i++ )
    {
        if( svs.clients[i].state >= CS_ACTIVE ) players++;
    }
...
void idServerCommunityServer::NET_Loop( void )
{
    ...
    if( ( actual_date - last_update ) > ( 60000 * 3 ) && community_stats.mapstatus != 2 )
    {
    
        offset += ::sprintf( &( buffer[offset] ), "{\"ST\":\"S\",\"GAME\":{\"PRT\":\"%d\",\"GID\":\"%s\",\"MAP\":\"%s\",\"STM\":\"%d\",\"GTY\":\"%d\",\"STS\":\"%d\"},\"STAT\":{\"TME\":\"%d\",\"PLY\":\"%d\",\"CPU\":\"%d\",\"MEM\":\"%d\"",
                             sv_cs_ServerPort->integer,
                             community_stats.game_id,
                             sv_mapname->string,
        ...
        for( i = 0; i < sv_maxclients->integer; i++ )
        {
            if( svs.clients[i].state >= CS_ACTIVE && svs.clients[i].cs_user != nullptr )
            {
                offset += ::sprintf( &( buffer[offset] ), ",\"PLN\":\"%s\"", svs.clients[i].cs_user->name );
            }
        }
