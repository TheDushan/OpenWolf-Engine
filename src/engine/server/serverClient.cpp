...
void idServerClientSystemLocal::GetChallenge(netadr_t from) {
    ...
    for(i = 0; i < MAX_CHALLENGES; i++, challenge++) {
        if(!challenge->connected &&
                networkSystem->CompareAdr(from, challenge->adr)) {
            wasfound = true;
    ...
    if(svs.authorizeAddress.type == NA_BAD) {
        ...
        if(developer->integer) {
            common->Printf("Sending getIpAuthorize for %s\n",
    ...
    if(svs.time - challenge->firstTime > AUTHORIZE_TIMEOUT) {
        ...
        if(sv_onlyVisibleClients->integer) {
            networkChainSystem->OutOfBandPrint(NS_SERVER, challenge->adr,
                                               "challengeResponse %i %i %i", challenge->challenge, clientChallenge,
                                               sv_onlyVisibleClients->integer);
        } else {
            networkChainSystem->OutOfBandPrint(NS_SERVER, challenge->adr,
                                               "challengeResponse %i %i", challenge->challenge, clientChallenge);
        }
...
void idServerClientSystemLocal::AuthorizeIpPacket(netadr_t from) {
    ...
    if(!networkSystem->CompareBaseAdr(from, svs.authorizeAddress)) {
        common->Printf("idServerClientSystemLocal::AuthorizeIpPacket: not from authorize server\n");
        return;
    }
    ...
    if(i == MAX_CHALLENGES) {
        common->Printf("idServerClientSystemLocal::AuthorizeIpPacket: challenge not found\n");
        return;
    }
    ...
    if(response == 0) {
        ...
        if(sv_onlyVisibleClients->integer) {
            networkChainSystem->OutOfBandPrint(NS_SERVER, challengeptr->adr,
                                               "challengeResponse %i %i", challengeptr->challenge,
                                               sv_onlyVisibleClients->integer);
        } else {
            networkChainSystem->OutOfBandPrint(NS_SERVER, svs.challenges[i].adr,
                                               "challengeResponse %i", challengeptr->challenge);
        }
    ...
    if(response == 1) {
        if(!reason) {
            networkChainSystem->OutOfBandPrint(NS_SERVER, challengeptr->adr,
                                               "authStatus %i 1", challengeptr->getChallengeCookie);
        } else {
            networkChainSystem->OutOfBandPrint(NS_SERVER, challengeptr->adr,
                                               "authStatus %i 1 %s", challengeptr->getChallengeCookie, reason);
        }
    ...
    if(response > 1) {
        if(!reason) {
            networkChainSystem->OutOfBandPrint(NS_SERVER, challengeptr->adr,
                                               "authStatus %i %i", challengeptr->getChallengeCookie, response);
        } else {
            networkChainSystem->OutOfBandPrint(NS_SERVER, challengeptr->adr,
...
void idServerClientSystemLocal::DirectConnect(netadr_t from) {
    ...
    if((static_cast<sint>(::strlen(ip)) + static_cast<sint>(::strlen(
                userinfo)) + 4) >= MAX_INFO_STRING) {
        networkChainSystem->OutOfBandPrint(NS_SERVER, from,
    ...
    if(!networkSystem->IsLocalAddress(from)) {
        ...
        if(!networkSystem->IsLANAddress(from)) {
            if((sv_minPing->value > 0) && ping < sv_minPing->value) {
                ...
                if(developer->integer) {
                    common->Printf("Client %i rejected on a too low ping\n", i);
                }
            ...
            if((sv_maxPing->value > 0) && ping > sv_maxPing->value) {
                ...
                if(developer->integer) {
                    common->Printf("Client %i rejected on a too high ping\n", i);
                }
    ...
    if(!networkSystem->IsLANAddress(from)) {
        ...
        for(i = 0; i < sv_maxclients->integer; i++) {
            if(svs.clients[i].netchan.remoteAddress.type != NA_BOT &&
                    ...
                    && networkSystem->CompareBaseAdr(svs.clients[i].netchan.remoteAddress,
                            from)) {
                connectingip++;
            }
        }
    ...
    if(!newcl) {
        ...
        if(!cl) {
            ...
            if(developer->integer) {
                common->Printf("Rejected a connection.\n");
            }
...
gotnewcl:
    ...
    *newcl = temp;
    ...
    if(denied) {
        ...
        if(developer->integer) {
            common->Printf("Game rejected a connection: %s.\n", denied);
        }
    ...
    if(developer->integer) {
        common->Printf("Going from CS_FREE to CS_CONNECTED for %s\n", newcl->name);
    }
...
void idServerClientSystemLocal::DropClient(client_t *drop,
    ...
    if(!isBot) {
        ...
        for(i = 0; i < MAX_CHALLENGES; i++, challenge++) {
            if(networkSystem->CompareAdr(drop->netchan.remoteAddress,
                                         challenge->adr)) {
    ...
    if(isBot) {
        ...
        if(developer->integer) {
            common->Printf("Going to CS_ZOMBIE for %s\n", drop->name);
        }
...
================
*/
void idServerClientSystemLocal::CreateClientGameStateMessage(
    ...
    if(developer->integer) {
        common->Printf("Sending %i bytes in gamestate to client: %li\n",
                       msg->cursize,
...
void idServerClientSystemLocal::SendClientGameState(client_t *client) {
    ...
    while(client->state && client->netchan.unsentFragments) {
        if(developer->integer) {
            common->Printf("idServerClientSystemLocal::SendClientGameState [2] for %s, writing out old fragments\n",
                           client->name);
        }
    ...
    if(developer->integer) {
        common->Printf("idServerClientSystemLocal::SendClientGameState() for %s\n",
                       client->name);
    }

    if(client->state != CS_PRIMED) {
        if(developer->integer) {
            common->Printf("Going from CS_CONNECTED to CS_PRIMED for %s\n",
                           client->name);
        }
    }
...
==================
*/
void idServerClientSystemLocal::ClientEnterWorld(client_t *client,
        usercmd_t *cmd) {
    sint clientNum;
    sharedEntity_t *ent;
    ...
    if(developer->integer) {
        common->Printf("Going from CS_PRIMED to CS_ACTIVE for %s\n", client->name);
    }
...
void idServerClientSystemLocal::StopDownload_f(client_t *cl) {
    if(*cl->downloadName) {
        if(developer->integer) {
            common->Printf("clientDownload: %d : file \"%s\" aborted\n",
...
void idServerClientSystemLocal::DoneDownload_f(client_t *cl) {
    ...
    if(developer->integer) {
        common->Printf("clientDownload: %s Done\n", cl->name);
    }
...
void idServerClientSystemLocal::NextDownload_f(client_t *cl) {
    ...
    if(block == cl->downloadClientBlock) {
        if(developer->integer) {
            common->Printf("clientDownload: %d : client acknowledge of block %d\n",
...
void idServerClientSystemLocal::WWWDownload_f(client_t *cl) {
    ...
    if(!cl->bWWWDl) {
        common->Printf("idServerClientSystemLocal::WWWDownload: unexpected wwwdl '%s' for client '%s'\n",
                       subcmd, cl->name);
        serverClientLocal.DropClient(cl,
                                     "idServerClientSystemLocal::WWWDownload: unexpected wwwdl");
        return;
    }
...
==================
*/
bool idServerClientSystemLocal::CheckFallbackURL(client_t *cl,
        msg_t *msg) {
...
==================
*/
void idServerClientSystemLocal::WriteDownloadToClient(client_t *cl,
        msg_t *msg) {
    ...
    if(!cl->download) {
        ...
        if(!sv_allowDownload->integer || idPack) {
            ...
            if(!::strstr(cl->downloadName, ".pk3")) {
                common->Printf("Not a pk3 %s\n", cl->downloadName);
                Q_vsprintf_s(errorMessage, sizeof(errorMessage), sizeof(errorMessage),
                             "Cannot download non-pk3 \"%s\"\n", cl->downloadName);
            } else if(::strstr(cl->downloadName, "..")) {
                common->Printf("Folder hack? %s\n", cl->downloadName);
                Q_vsprintf_s(errorMessage, sizeof(errorMessage), sizeof(errorMessage),
                             "Cannot download bad folder \"%s\"\n", cl->downloadName);
            } else if(idPack) {
                common->Printf("clientDownload: %d : \"%s\" cannot download id pk3 files\n",
                               static_cast<sint>(cl - svs.clients), cl->downloadName);
                Q_vsprintf_s(errorMessage, sizeof(errorMessage), sizeof(errorMessage),
                             "Cannot autodownload official pk3 file \"%s\"", cl->downloadName);
            } else {
                common->Printf("clientDownload: %d : \"%s\" download disabled",
                               static_cast<sint>(cl - svs.clients), cl->downloadName);
        ...
        if(sv_wwwDownload->integer) {
            if(cl->bDlOK) {
                if(!cl->bFallback) {
                    fileHandle_t handle;
                    sint downloadSize = fileSystem->SV_FOpenFileRead(cl->downloadName,
                                        &handle);
        ...
        // find file
        cl->bWWWDl = false;
        cl->downloadSize = fileSystem->SV_FOpenFileRead(cl->downloadName,
                           &cl->download);
    ...
    while(cl->downloadCurrentBlock - cl->downloadClientBlock <
            MAX_DOWNLOAD_WINDOW && cl->downloadSize != cl->downloadCount) {
        ...
        cl->downloadBlockSize[curindex] = fileSystem->Read(
    ...
    while(blockspersnap--) {
        ...
        if(developer->integer) {
            common->Printf("clientDownload: %d : writing block %d\n",
...
void idServerClientSystemLocal::VerifyPaks_f(client_t *cl) {
    sint nChkSum1, nChkSum2, nClientPaks, nServerPaks, i, j,
         nClientChkSum[1024], nServerChkSum[1024];
    pointer pPaks, pArg;
    ...
    if(sv_pure->integer != 0) {
        ...
        bGood = (bool)(fileSystem->FileIsInPAK(idsystem->GetDLLName("cgame"),
                                               &nChkSum1) == 1);

        if(bGood) {
            bGood = (bool)(fileSystem->FileIsInPAK(idsystem->GetDLLName("gui"),
                                                   &nChkSum2) == 1);
        ...
        if(!pArg) {
            ...
            if(::atoi(pArg) < sv.checksumFeedServerId) {
                if(developer->integer) {
                    common->Printf("ignoring outdated cp command from client %s\n", cl->name);
                }
        ...
        if(bGood) {
            ...
            serverMainSystem->SendServerCommand(cl, "disconnect \"%s\"",
                                                "This is a pure server. This is caused by corrupted or missing files. Try turning on AutoDownload.");
...
==================
*/
bool idServerClientSystemLocal::CheckFunstuffExploit(valueType *userinfo,
        valueType *key) {
...
void idServerClientSystemLocal::UserinfoChanged(client_t *cl) {
    ...
    //if (developer->integer) {
    //    common->Printf("Maintain IP in userinfo for '%s'\n", cl->name);
    //}
    ...
    for(i = 0; i < strlen(val); i++) {
        if(!isalnum(val[i])) {
            ...
            common->Printf("Cleared malformed cl_guid from %s.\n",
                           networkSystem->AdrToString(cl->netchan.remoteAddress));
            break;
        }
    }
    ...
    if(CheckFunstuffExploit(cl->userinfo, "funfree") ||
        ...
        common->Printf("funstuff exploit attempt from %s\n",
                       networkSystem->AdrToString(cl->netchan.remoteAddress));
    }
}
...
void idServerClientSystemLocal::UpdateUserinfo_f(client_t *cl) {
    ...
    if((sv_floodProtect->integer) && (cl->state >= CS_ACTIVE) &&
        ...
        serverMainSystem->SendServerCommand(cl,
                                            "print \"^7Command ^1delayed^7 due to sv_floodprotect.\"");
        return;
    }
    ...
    serverInitSystem->GetConfigstring(CS_PLAYERS + (cl - svs.clients), info,
                                      sizeof(info));
...
==================
*/
void idServerClientSystemLocal::ExecuteClientCommand(client_t *cl,
        pointer s, bool clientOK, bool premaprestart) {
    ucmd_t *u;
    ...
    if(developer->integer) {
        common->Printf("idServerClientSystemLocal::ExecuteClientCommand: %s\n", s);
    }
    ...
    if(clientOK) {
        ...
        if(!u->name && sv.state == SS_GAME) {
            ...
            // q3cbufexec fix
            if((!Q_stricmp(cmdSystem->Argv(0), "callvote") ||
                    !Q_stricmp(cmdSystem->Argv(0), "callteamvote"))) {
                if(strpbrk(cmdSystem->Argv(1), ";\n\r") ||
                        strpbrk(cmdSystem->Argv(2), ";\n\r")) {
                    common->Printf("Callvote from %s (client #%i, %s): %s\n", cl->name,
            ...
            // teamcmd crash fix
            if(!Q_stricmp(cmdSystem->Argv(0), "team") &&
                    (!Q_stricmp(cmdSystem->Argv(1), "follow1") ||
                     !Q_stricmp(cmdSystem->Argv(1), "follow2"))) {
                return;
            }

            if(!Q_stricmp("stats", cmdSystem->Argv(0))) {
                if(cl - svs.clients != serverGameSystem->GameClientNum(
                            cl - svs.clients)->clientNum) {
                    common->Printf("Stats command exploit attempt from %s\n",
                                   networkSystem->AdrToString(cl->netchan.remoteAddress));
                    serverMainSystem->SendServerCommand(cl,
                                                        "print \"^7Stats command exploit attempt detected. This has been ^1reported^7.\n\"");
                    return;
                }
            }
            ...
            if(argsFromOneMaxlen >= 0) {
                ...
                if(exploitDetected) {
                    common->Printf("Buffer overflow exploit radio/say, possible attempt from %s\n",
                                   networkSystem->AdrToString(cl->netchan.remoteAddress));
                    serverMainSystem->SendServerCommand(cl,
                                                        "print \"Chat dropped due to message length constraints.\n\"");
                    return;
                }
        ...
        if(developer->integer) {
            common->Printf("client text ignored for %s: %s\n", cl->name,
                           cmdSystem->Argv(0));
        }
    } else if(!foundCommand) {
        if(developer->integer) {
            common->Printf("client text ignored for %s\n", cl->name);
        }
    }
}
...
===============
*/
bool idServerClientSystemLocal::ClientCommand(client_t *cl, msg_t *msg,
        bool premaprestart) {
    sint seq;
    pointer s;
    ...
    if(developer->integer) {
        common->Printf("clientCommand: %s : %i : %s\n", cl->name, seq, s);
    }
    ...
    if(!Q_strncmp("ws", s, 2)) {
        ...
        if(strlen(s) > 2) {
            ...
            if(idx < 0 ||
                    idx > 128) { //Using 128 because i doubt this may be higher than 128, no matter which mod is used
    ...
    if(!Q_strncmp("team", s, 4) || !Q_strncmp("setspawnpt", s, 10) ||
        ...
        //if (developer->integer) {
        //common->Printf( "Skipping flood protection for: %s\n", s );
        //}
        floodprotect = false;
    }
...
void idServerClientSystemLocal::UserMove(client_t *cl, msg_t *msg,
    ...
    if(sv_pure->integer != 0 && cl->pureAuthentic == false &&
        ...
        if(cl->state == CS_ACTIVE) {
            ...
            if(developer->integer) {
                common->Printf("%s: didn't get cp command, resending gamestate\n",
                               cl->name);
            }
...
===================
*/
void idServerClientSystemLocal::ExecuteClientMessage(client_t *cl,
        msg_t *msg) {
    sint c, serverId;
    ...
    if(serverId != sv.serverId && !*cl->downloadName &&
        ...
        if(serverId >= sv.restartedServerId &&
            ...
            if(developer->integer) {
                common->Printf("%s : ignoring pre map_restart / outdated client message\n",
                               cl->name);
            }
        ...
        if(cl->state != CS_ACTIVE &&
            ...
            if(developer->integer) {
                common->Printf("%s : dropped gamestate, resending\n", cl->name);
            }
        ...
        if(cl->oldServerTime && serverId == sv.serverId) {
            if(developer->integer) {
                common->Printf("%s acknowledged gamestate\n", cl->name);
            }
