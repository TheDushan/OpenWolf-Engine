...
static float32 s_cloudTexCoords[6][SKY_SUBDIVISIONS + 1][SKY_SUBDIVISIONS +
        1][2];
static float32 s_cloudTexP[6][SKY_SUBDIVISIONS + 1][SKY_SUBDIVISIONS + 1];
...
static float32  sky_mins[2][6], sky_maxs[2][6];
static float32  sky_min, sky_max;
...
================
*/
static void AddSkyPolygon(sint nump, vec3_t vecs) {
    sint        i, j;
    vec3_t  v, av;
    float32 s, t, dv;
    sint        axis;
...
================
*/
static void ClipSkyPolygon(sint nump, vec3_t vecs, sint stage) {
    float32    *norm;
    float32    *v;
    bool    front, back;
    float32 d, e;
    float32 dists[MAX_CLIP_VERTS];
    sint        sides[MAX_CLIP_VERTS];
    vec3_t  newv[2][MAX_CLIP_VERTS];
    sint        newc[2];
    sint        i, j;

    if(nump > MAX_CLIP_VERTS - 2) {
        common->Error(ERR_DROP, "ClipSkyPolygon: MAX_CLIP_VERTS");
    }
...
*/
static void MakeSkyVec(float32 s, float32 t, sint axis, float32 outSt[2],
                       vec3_t outXYZ) {
    // 1 = s, 2 = t, 3 = 2048
    static sint st_to_vec[6][3] = {
        {3, -1, 2},
        { -3, 1, 2},
...
static sint sky_texorder[6] = {0, 2, 1, 3, 4, 5};
static vec3_t   s_skyPoints[SKY_SUBDIVISIONS + 1][SKY_SUBDIVISIONS + 1];
static float32  s_skyTexCoords[SKY_SUBDIVISIONS + 1][SKY_SUBDIVISIONS +
        1][2];

static void DrawSkySide(struct image_s *image, const sint mins[2],
                        const sint maxs[2]) {
    sint s, t;
    sint firstVertex = tess.numVertexes;
    //sint firstIndex = tess.numIndexes;
    vec4_t color;
    ...
    for(t = mins[1] + HALF_SKY_SUBDIVISIONS;
        ...
        for(s = mins[0] + HALF_SKY_SUBDIVISIONS;
            ...
            if(tess.numVertexes >= SHADER_MAX_VERTEXES) {
                common->Error(ERR_DROP, "SHADER_MAX_VERTEXES hit in DrawSkySideVBO()");
            }
        }
    }
    ...
    for(t = 0; t < maxs[1] - mins[1]; t++) {
        for(s = 0; s < maxs[0] - mins[0]; s++) {
            if(tess.numIndexes + 6 >= SHADER_MAX_INDEXES) {
                common->Error(ERR_DROP, "SHADER_MAX_INDEXES hit in DrawSkySideVBO()");
            }

            tess.indexes[tess.numIndexes++] =  s +       t      *
                                               (maxs[0] - mins[0] + 1) + firstVertex;
            tess.indexes[tess.numIndexes++] =  s + (t + 1) * (maxs[0] - mins[0] + 1) +
                                               firstVertex;
            tess.indexes[tess.numIndexes++] = (s + 1) +  t      *
                                              (maxs[0] - mins[0] + 1) + firstVertex;
            ...
            tess.indexes[tess.numIndexes++] =  s + (t + 1) * (maxs[0] - mins[0] + 1) +
                                               firstVertex;
            tess.indexes[tess.numIndexes++] = (s + 1) + (t + 1) *
                                              (maxs[0] - mins[0] + 1) + firstVertex;
        }
    }
...
static void DrawSkyBox(shader_t *shader) {
    ...
    for(i = 0 ; i < 6 ; i++) {
        sint sky_mins_subd[2], sky_maxs_subd[2];
        sint s, t;
        ...
        for(t = sky_mins_subd[1] + HALF_SKY_SUBDIVISIONS;
            ...
            for(s = sky_mins_subd[0] + HALF_SKY_SUBDIVISIONS;
                ...
                MakeSkyVec((s - HALF_SKY_SUBDIVISIONS) / static_cast<float32>
                           (HALF_SKY_SUBDIVISIONS),
...
static void FillCloudySkySide(const sint mins[2], const sint maxs[2],
                              bool addIndexes) {
    sint s, t;
    sint vertexStart = tess.numVertexes;
    sint tHeight, sWidth;
    ...
    for(t = mins[1] + HALF_SKY_SUBDIVISIONS;
        ...
        for(s = mins[0] + HALF_SKY_SUBDIVISIONS;
                s <= maxs[0] + HALF_SKY_SUBDIVISIONS; s++) {
            VectorAdd(s_skyPoints[t][s], backEnd.viewParms.orientation.origin,
            ...
            if(tess.numVertexes >= SHADER_MAX_VERTEXES) {
                common->Error(ERR_DROP, "SHADER_MAX_VERTEXES hit in FillCloudySkySide()");
            }
        }
    }
...
static void FillCloudBox(const shader_t *shader, sint stage) {
    sint i;
    ...
    for(i = 0; i < 6; i++) {
        ...
        sky_mins_subd[0] = static_cast<sint>(sky_mins[0][i] *
                                             HALF_SKY_SUBDIVISIONS);
        sky_mins_subd[1] = static_cast<sint>(sky_mins[1][i] *
                                             HALF_SKY_SUBDIVISIONS);
        sky_maxs_subd[0] = static_cast<sint>(sky_maxs[0][i] *
                                             HALF_SKY_SUBDIVISIONS);
        sky_maxs_subd[1] = static_cast<sint>(sky_maxs[1][i] *
                                             HALF_SKY_SUBDIVISIONS);
