...
=================
*/
void FBO_CreateBuffer(FBO_t *fbo, sint format, sint index,
                      sint multisample) {
    uint *pRenderBuffer;
    GLenum attachment;
    bool absent;
    ...
    if(absent) {
        if(attachment == 0) {
            qglNamedFramebufferRenderbufferEXT(fbo->frameBuffer, GL_DEPTH_ATTACHMENT,
                                               GL_RENDERBUFFER, *pRenderBuffer);
            qglNamedFramebufferRenderbufferEXT(fbo->frameBuffer, GL_STENCIL_ATTACHMENT,
                                               GL_RENDERBUFFER, *pRenderBuffer);
        } else {
            qglNamedFramebufferRenderbufferEXT(fbo->frameBuffer, attachment,
                                               GL_RENDERBUFFER, *pRenderBuffer);
        }
    }
}
...
void idRenderSystemLocal::FBOInit(void) {
    ...
    //
    {
        tr.genericFbo = FBO_Create("_generic", tr.genericFBOImage->width,
                                   tr.genericFBOImage->height);
        FBO_Bind(tr.genericFbo);
        FBO_AttachImage(tr.genericFbo, tr.genericFBOImage, GL_COLOR_ATTACHMENT0,
                        0);
        R_CheckFBO(tr.genericFbo);
    }

    {
        tr.genericFbo2 = FBO_Create("_generic2", tr.genericFBO2Image->width,
                                    tr.genericFBO2Image->height);
        FBO_Bind(tr.genericFbo2);
        FBO_AttachImage(tr.genericFbo2, tr.genericFBO2Image,
    ...
    //
    {
        tr.bloomRenderFBO[0] = FBO_Create("_bloom0",
                                          tr.bloomRenderFBOImage[0]->width, tr.bloomRenderFBOImage[0]->height);
        FBO_Bind(tr.bloomRenderFBO[0]);
        FBO_AttachImage(tr.bloomRenderFBO[0], tr.bloomRenderFBOImage[0],
                        GL_COLOR_ATTACHMENT0, 0);
        R_CheckFBO(tr.bloomRenderFBO[0]);


        tr.bloomRenderFBO[1] = FBO_Create("_bloom1",
                                          tr.bloomRenderFBOImage[1]->width, tr.bloomRenderFBOImage[1]->height);
        FBO_Bind(tr.bloomRenderFBO[1]);
        FBO_AttachImage(tr.bloomRenderFBO[1], tr.bloomRenderFBOImage[1],
                        GL_COLOR_ATTACHMENT0, 0);
        R_CheckFBO(tr.bloomRenderFBO[1]);


        tr.bloomRenderFBO[2] = FBO_Create("_bloom2",
                                          tr.bloomRenderFBOImage[2]->width, tr.bloomRenderFBOImage[2]->height);
        FBO_Bind(tr.bloomRenderFBO[2]);
        FBO_AttachImage(tr.bloomRenderFBO[2], tr.bloomRenderFBOImage[2],
                        GL_COLOR_ATTACHMENT0, 0);
    ...
    //
    {
        tr.anamorphicRenderFBO[0] = FBO_Create("_anamorphic0",
                                               tr.anamorphicRenderFBOImage[0]->width,
                                               tr.anamorphicRenderFBOImage[0]->height);
        FBO_AttachImage(tr.anamorphicRenderFBO[0], tr.anamorphicRenderFBOImage[0],
                        GL_COLOR_ATTACHMENT0, 0);
        R_CheckFBO(tr.anamorphicRenderFBO[0]);

        tr.anamorphicRenderFBO[1] = FBO_Create("_anamorphic1",
                                               tr.anamorphicRenderFBOImage[1]->width,
                                               tr.anamorphicRenderFBOImage[1]->height);
        FBO_AttachImage(tr.anamorphicRenderFBO[1], tr.anamorphicRenderFBOImage[1],
                        GL_COLOR_ATTACHMENT0, 0);
        R_CheckFBO(tr.anamorphicRenderFBO[1]);

        tr.anamorphicRenderFBO[2] = FBO_Create("_anamorphic2",
                                               tr.anamorphicRenderFBOImage[2]->width,
                                               tr.anamorphicRenderFBOImage[2]->height);
        FBO_AttachImage(tr.anamorphicRenderFBO[2], tr.anamorphicRenderFBOImage[2],
                        GL_COLOR_ATTACHMENT0, 0);
        R_CheckFBO(tr.anamorphicRenderFBO[2]);
    }

    {
        tr.renderFbo = FBO_Create("_render", tr.renderDepthImage->width,
        ...
        FBO_AttachImage(tr.renderFbo, tr.normalDetailedImage,
                        GL_COLOR_ATTACHMENT0_EXT, 0);
    ...
    if(tr.sunRaysImage) {
        tr.sunRaysFbo = FBO_Create("_sunRays", tr.renderDepthImage->width,
                                   tr.renderDepthImage->height);
        FBO_AttachImage(tr.sunRaysFbo, tr.sunRaysImage, GL_COLOR_ATTACHMENT0, 0);
        FBO_AttachImage(tr.sunRaysFbo, tr.renderDepthImage, GL_DEPTH_ATTACHMENT,
                        0);
    ...
    if(MAX_DRAWN_PSHADOWS && tr.pshadowMaps[0]) {
        for(i = 0; i < MAX_DRAWN_PSHADOWS; i++) {
            ...
            FBO_AttachImage(tr.pshadowFbos[i], tr.pshadowMaps[i], GL_DEPTH_ATTACHMENT,
                            0);
    ...
    if(tr.sunShadowDepthImage[0]) {
        for(i = 0; i < 4; i++) {
            ...
            FBO_AttachImage(tr.sunShadowFbo[i], tr.sunShadowDepthImage[i],
                            GL_DEPTH_ATTACHMENT, 0);
    ...
    if(tr.calcLevelsImage) {
        tr.calcLevelsFbo = FBO_Create("_calclevels", tr.calcLevelsImage->width,
                                      tr.calcLevelsImage->height);
        FBO_AttachImage(tr.calcLevelsFbo, tr.calcLevelsImage, GL_COLOR_ATTACHMENT0,
                        0);
    ...
    if(tr.hdrDepthImage) {
        tr.hdrDepthFbo = FBO_Create("_hdrDepth", tr.hdrDepthImage->width,
                                    tr.hdrDepthImage->height);
        FBO_AttachImage(tr.hdrDepthFbo, tr.hdrDepthImage, GL_COLOR_ATTACHMENT0, 0);
        R_CheckFBO(tr.hdrDepthFbo);
    }

    if(tr.screenSsaoImage) {
        tr.screenSsaoFbo = FBO_Create("_screenssao", tr.screenSsaoImage->width,
                                      tr.screenSsaoImage->height);
        FBO_AttachImage(tr.screenSsaoFbo, tr.screenSsaoImage, GL_COLOR_ATTACHMENT0,
                        0);
        R_CheckFBO(tr.screenSsaoFbo);
    }

    if(tr.renderCubeImage != nullptr) {
        tr.renderCubeFbo = FBO_Create("_renderCubeFbo", tr.renderCubeImage->width,
                                      tr.renderCubeImage->height);
        FBO_AttachImage(tr.renderCubeFbo, tr.renderCubeImage, GL_COLOR_ATTACHMENT0,
                        0);
    ...
    if(tr.renderCubeImage != nullptr) {
        tr.preFilterEnvMapFbo = FBO_Create("_preFilterEnvMapFbo",
                                           tr.renderCubeImage->width, tr.renderCubeImage->height);
        FBO_AttachImage(tr.preFilterEnvMapFbo, tr.prefilterEnvMapImage,
                        GL_COLOR_ATTACHMENT0, 0);
...
void FBO_BlitFromTexture(struct image_s *src, vec4_t inSrcTexCorners,
    ...
    GLSL_SetUniformMat4(shaderProgram, UNIFORM_MODELVIEWPROJECTIONMATRIX,
                        projection);
    GLSL_SetUniformVec4(shaderProgram, UNIFORM_COLOR, color);
    GLSL_SetUniformVec2(shaderProgram, UNIFORM_INVTEXRES, invTexRes);
    GLSL_SetUniformVec2(shaderProgram, UNIFORM_AUTOEXPOSUREMINMAX,
                        tr.refdef.autoExposureMinMax);
    GLSL_SetUniformVec3(shaderProgram, UNIFORM_TONEMINAVGMAXLINEAR,
...
void FBO_Blit(FBO_t *src, ivec4_t inSrcBox, vec2_t srcTexScale, FBO_t *dst,
    ...
    if(inSrcBox) {
        srcTexCorners[0] =  inSrcBox[0]                / static_cast<float32>
                            (src->width);
        srcTexCorners[1] = (inSrcBox[1] + inSrcBox[3]) / static_cast<float32>
                           (src->height);
        srcTexCorners[2] = (inSrcBox[0] + inSrcBox[2]) / static_cast<float32>
                           (src->width);
        srcTexCorners[3] =  inSrcBox[1]                / static_cast<float32>
                            (src->height);
    } else {
        VectorSet4(srcTexCorners, 0.0f, 0.0f, 1.0f, 1.0f);
    }

    FBO_BlitFromTexture(src->colorImage[0], srcTexCorners, srcTexScale, dst,
                        dstBox, shaderProgram, color, blend | GLS_DEPTHTEST_DISABLE);
}
...
void FBO_FastBlit(FBO_t *src, ivec4_t srcBox, FBO_t *dst, ivec4_t dstBox,
    ...
    if(!srcBox) {
        ...
        VectorSet4(srcBoxFinal, srcBox[0], srcBox[1], srcBox[0] + srcBox[2],
                   srcBox[1] + srcBox[3]);
    }
    ...
    if(!dstBox) {
        ...
        VectorSet4(dstBoxFinal, dstBox[0], dstBox[1], dstBox[0] + dstBox[2],
                   dstBox[1] + dstBox[3]);
    }
