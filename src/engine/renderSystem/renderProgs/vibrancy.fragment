...
{
	vec4	res;
	vec3	origcolor = texture2D(u_TextureMap, var_TexCoords.xy).rgb;
	...
	res.rgb = mix(vec3(luma), origcolor.rgb, (1.0 + (var_Local0.x * (1.0 - (sign(var_Local0.x) * color_saturation))))); 	//Extrapolate between luma and original by 1 + (1-saturation) - current
