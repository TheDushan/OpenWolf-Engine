...
static void Matrix34Multiply(const float32 *a, const float32 *b,
                             float32 *out) {
...
static void QuatSlerp(const quat_t from, const quat_t _to,
                      float32 fraction, quat_t out) {
    float32 angle, cosAngle, sinAngle, backlerp, lerp;
    quat_t to;

    // cos() of angle
    cosAngle = from[0] * _to[0] + from[1] * _to[1] + from[2] * _to[2] + from[3]
               * _to[3];
...
static vec_t QuatNormalize2(const quat_t v, quat_t out) {
    ...
    if(length) {
        ...
        ilength = 1 / static_cast<float32>(sqrt(length));
...
=================
*/
bool R_LoadIQM(model_t *mod, void *buffer, sint filesize,
               pointer mod_name) {
    iqmHeader_t *header;
...
#define LL(x) x=LittleLong(x)
    ...
    if(header->num_meshes) {
        ...
        vertexarray = (iqmVertexArray_t *)(reinterpret_cast<uchar8 *>
        ...
        for(i = 0; i < header->num_vertexarrays; i++, vertexarray++) {
            ...
            switch(vertexarray->format) {
                ...
                case IQM_UBYTE:
                    ...
                    if(IQM_CheckRange(header, vertexarray->offset, n, sizeof(uchar8))) {
                        return false;
                    }
                ...
                case IQM_FLOAT:
                    ...
                    // 4-uchar8 swap
                    if(IQM_CheckRange(header, vertexarray->offset, n, sizeof(float32))) {
                        return false;
                    }

                    intPtr = reinterpret_cast<sint *>(reinterpret_cast<uchar8 *>
                                                      (header) + vertexarray->offset);
            ...
            switch(vertexarray->type) {
                ...
                case IQM_BLENDINDEXES:
                    ...
                    blendIndexes = reinterpret_cast<uchar8 *>(header) + vertexarray->offset;
                    break;
                ...
                case IQM_BLENDWEIGHTS:
                    ...
                    if(vertexarray->format == IQM_FLOAT) {
                        blendWeights.f = reinterpret_cast< float32 * >((reinterpret_cast<uchar8 *>
                                         (header) + vertexarray->offset));
                    } else {
                        blendWeights.b = reinterpret_cast<uchar8 *>(header) + vertexarray->offset;
                    }
        ...
        triangle = (iqmTriangle_t *)(reinterpret_cast<uchar8 *>
                                     (header) + header->ofs_triangles);
        ...
        mesh = (iqmMesh_t *)(reinterpret_cast<uchar8 *>(header) +
                             header->ofs_meshes);
        ...
        for(i = 0; i < header->num_meshes; i++, mesh++) {
            ...
            if(mesh->name < header->num_text) {
                Q_strncpyz(meshName, reinterpret_cast<valueType *>(header) +
                           header->ofs_text + mesh->name, sizeof(meshName));
            } else {
                meshName[0] = '\0';
            }
            ...
            if(header->num_joints) {
                for(j = 0; j < mesh->num_vertexes; j++) {
                    ...
                    for(k = 0; k < j; k++) {
                        ...
                        if(*reinterpret_cast<sint *>(& blendIndexes[4 * influence]) !=
                                *reinterpret_cast<sint *>(& blendIndexes[4 * vtx])) {
                            continue;
                        }
                        ...
                        if(vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_FLOAT) {
                            ...
                            if(*reinterpret_cast<sint *>(& blendWeights.b[4 * influence]) ==
                                    *reinterpret_cast<sint *>(& blendWeights.b[4 * vtx])) {
                                break;
                            }
                        }
                    }
    ...
    if(header->num_joints) {
        ...
        joint = (iqmJoint_t *)(reinterpret_cast<uchar8 *>(header) +
                               header->ofs_joints);
        ...
        for(i = 0; i < header->num_joints; i++, joint++) {
            ...
            if(joint->parent < -1 ||
                    joint->parent >= static_cast<sint>(header->num_joints) ||
                    joint->name >= static_cast<sint>(header->num_text)) {
                return false;
            }

            joint_names += strlen(reinterpret_cast<valueType *>(header) +
                                  header->ofs_text +
                                  joint->name) + 1;
        }
    }
    ...
    if(header->num_poses) {
        ...
        pose = (iqmPose_t *)(reinterpret_cast<uchar8 *>(header) +
                             header->ofs_poses);
    ...
    if(header->ofs_bounds) {
        ...
        bounds = (iqmBounds_t *)(reinterpret_cast<uchar8 *>(header) +
                                 header->ofs_bounds);
    ...
    if(header->num_meshes) {
        ...
        if(allocateInfluences) {
            ...
            size += allocateInfluences * 4 * sizeof(
                        uchar8);         // influenceBlendIndexes

            if(vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_UBYTE) {
                size += allocateInfluences * 4 * sizeof(
                            uchar8);     // influenceBlendWeights
            } else if(vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_FLOAT) {
                size += allocateInfluences * 4 * sizeof(
                            float32);    // influenceBlendWeights
            }
        }
    }
    ...
    if(header->num_joints) {
        ...
        size += header->num_joints * 12 * sizeof(
                    float32);   // bind joint matricies
        size += header->num_joints * 12 * sizeof(
                    float32);   // inverse bind joint matricies
    }

    if(header->num_poses) {
        size += header->num_poses * header->num_frames * sizeof(
    ...
    // fill header
    iqmData->num_vertexes = (header->num_meshes > 0) ? header->num_vertexes :
                            0;
    iqmData->num_triangles = (header->num_meshes > 0) ? header->num_triangles :
                             0;
    ...
    dataPtr = reinterpret_cast<uchar8 *>(iqmData) + sizeof(iqmData_t);

    if(header->num_meshes) {
        iqmData->surfaces = (struct srfIQModel_s *)dataPtr;
        dataPtr += header->num_meshes * sizeof(srfIQModel_t);

        iqmData->triangles = reinterpret_cast<sint *>(dataPtr);
        dataPtr += header->num_triangles * 3 * sizeof(sint);         // triangles

        iqmData->positions = reinterpret_cast< float32 * >(dataPtr);
        dataPtr += header->num_vertexes * 3 * sizeof(float32);   // positions

        iqmData->texcoords = reinterpret_cast<float32 *>(dataPtr);
        dataPtr += header->num_vertexes * 2 * sizeof(float32);   // texcoords

        iqmData->normals = reinterpret_cast<float32 *>(dataPtr);
        dataPtr += header->num_vertexes * 3 * sizeof(float32);   // normals

        if(vertexArrayFormat[IQM_TANGENT] != -1) {
            iqmData->tangents = reinterpret_cast<float32 *>(dataPtr);
            dataPtr += header->num_vertexes * 4 * sizeof(float32);   // tangents
        }

        if(vertexArrayFormat[IQM_COLOR] != -1) {
            iqmData->colors = reinterpret_cast<uchar8 *>(dataPtr);
            dataPtr += header->num_vertexes * 4 * sizeof(uchar8);        // colors
        }

        if(allocateInfluences) {
            iqmData->influences = reinterpret_cast<sint *>(dataPtr);
            dataPtr += header->num_vertexes * sizeof(sint);              // influences

            iqmData->influenceBlendIndexes = reinterpret_cast<uchar8 *>(dataPtr);
            dataPtr += allocateInfluences * 4 * sizeof(
                           uchar8);          // influenceBlendIndexes

            if(vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_UBYTE) {
                iqmData->influenceBlendWeights.b = reinterpret_cast<uchar8 *>(dataPtr);
                dataPtr += allocateInfluences * 4 * sizeof(
                               uchar8);      // influenceBlendWeights
            } else if(vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_FLOAT) {
                iqmData->influenceBlendWeights.f = reinterpret_cast<float32 *>(dataPtr);
                dataPtr += allocateInfluences * 4 * sizeof(
                               float32);     // influenceBlendWeights
            }
        }
    ...
    if(header->num_joints) {
        iqmData->jointNames = reinterpret_cast<valueType *>(dataPtr);
        dataPtr += joint_names;                             // joint names

        iqmData->jointParents = reinterpret_cast<sint *>(dataPtr);
        dataPtr += header->num_joints * sizeof(sint);        // joint parents

        iqmData->bindJoints = reinterpret_cast<float32 *>(dataPtr);
        dataPtr += header->num_joints * 12 * sizeof(
                       float32);        // bind joint matricies

        iqmData->invBindJoints = reinterpret_cast<float32 *>(dataPtr);
        dataPtr += header->num_joints * 12 * sizeof(
                       float32);        // inverse bind joint matricies
    }
    ...
    if(header->ofs_bounds) {
        iqmData->bounds = reinterpret_cast<float32 *>(dataPtr);
    ...
    if(header->num_meshes) {
        ...
        str = reinterpret_cast<valueType *>(header) + header->ofs_text;
            ...
            switch(vertexarray->type) {
                case IQM_POSITION:
                    ::memcpy(iqmData->positions,
                             reinterpret_cast<uchar8 *>(header) + vertexarray->offset,
                             n * sizeof(float32));
                    break;
            ...
            for(i = 0; i < header->num_meshes; i++, surface++) {
                ...
                for(j = 0; j < surface->num_vertexes; j++) {
                    ...
                    for(k = 0; k < surface->num_influences; k++) {
                        ...
                        if(*reinterpret_cast<sint *>(& iqmData->influenceBlendIndexes[4 *
                                                     influence]) != *reinterpret_cast<sint *>(& blendIndexes[4 * vtx])) {
                            continue;
                        }

                        if(vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_FLOAT) {
                            if(iqmData->influenceBlendWeights.f[4 * influence + 0] == blendWeights.f[4
                                    * vtx + 0] &&
                                    iqmData->influenceBlendWeights.f[4 * influence + 1] == blendWeights.f[4 *
                                            vtx + 1] &&
                                    iqmData->influenceBlendWeights.f[4 * influence + 2] == blendWeights.f[4 *
                                            vtx + 2] &&
                                    iqmData->influenceBlendWeights.f[4 * influence + 3] == blendWeights.f[4 *
                                            vtx + 3]) {
                                break;
                            }
                        } else {
                            if(*reinterpret_cast<sint *>(& iqmData->influenceBlendWeights.b[4 *
                                                         influence]) == *reinterpret_cast<sint *>(& blendWeights.b[4 * vtx])) {
                                break;
                            }
                        }
                    }
                    ...
                    if(k == surface->num_influences) {
                        ...
                        iqmData->influenceBlendIndexes[4 * influence + 0] = blendIndexes[4 * vtx +
                                0];
                        iqmData->influenceBlendIndexes[4 * influence + 1] = blendIndexes[4 * vtx +
                                1];
                        iqmData->influenceBlendIndexes[4 * influence + 2] = blendIndexes[4 * vtx +
                                2];
                        iqmData->influenceBlendIndexes[4 * influence + 3] = blendIndexes[4 * vtx +
                                3];

                        if(vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_FLOAT) {
                            iqmData->influenceBlendWeights.f[4 * influence + 0] = blendWeights.f[4 *
                                    vtx + 0];
                            iqmData->influenceBlendWeights.f[4 * influence + 1] = blendWeights.f[4 *
                                    vtx + 1];
                            iqmData->influenceBlendWeights.f[4 * influence + 2] = blendWeights.f[4 *
                            ...
                            iqmData->influenceBlendWeights.b[4 * influence + 0] = blendWeights.b[4 *
                                    vtx + 0];
                            iqmData->influenceBlendWeights.b[4 * influence + 1] = blendWeights.b[4 *
                                    vtx + 1];
                            iqmData->influenceBlendWeights.b[4 * influence + 2] = blendWeights.b[4 *
                                    vtx + 2];
                            iqmData->influenceBlendWeights.b[4 * influence + 3] = blendWeights.b[4 *
                                    vtx + 3];
                        }
    ...
    if(header->num_joints) {
        ...
        for(i = 0; i < header->num_joints; i++, joint++) {
            valueType *name = reinterpret_cast<valueType *>(header) + header->ofs_text
                              +
                              joint->name;
        ...
        for(i = 0; i < header->num_joints; i++, joint++) {
            ...
            if(joint->parent >= 0) {
                ...
                mat += 12;
                Matrix34Multiply(invBaseFrame, iqmData->invBindJoints + 12 * joint->parent,
                                 matInv);
                matInv += 12;
            } else {
    ...
    if(header->num_poses) {
        ...
        framedata = reinterpret_cast<uchar16 *>(reinterpret_cast<uchar8 *>
                                                (header) + header->ofs_frames);
    ...
    if(header->ofs_bounds) {
        ...
        for(i = 0; i < header->num_vertexes; i++) {
            AddPointToBounds(&iqmData->positions[i * 3], &iqmData->bounds[0],
                             &iqmData->bounds[3]);
        }
    }

    // Create VAO surfaces
    if(iqmData->num_surfaces &&
            iqmData->num_joints <= glRefConfig.glslMaxAnimatedBones) {
        ...
        for(i = 0; i < iqmData->num_surfaces; i++, vaoSurf++, surf++) {
            ...
            for(j = 0; j < surf->num_vertexes; j++) {
                ...
                // normal
                R_VaoPackNormal(reinterpret_cast<schar16 *>(data) + dataOfs,
                                &iqmData->normals[vtx * 3]);
                dataOfs += sizeof(schar16) * 4;

                // tangent
                R_VaoPackTangent(reinterpret_cast<schar16 *>(data) + dataOfs,
                                 &iqmData->tangents[vtx * 4]);
                dataOfs += sizeof(schar16) * 4;
            ...
            if(iqmData->num_joints) {
                ...
                vaoSurf->vao->attribs[ATTR_INDEX_BONE_INDEXES].offset =
                    offset_blendindexes;
                vaoSurf->vao->attribs[ATTR_INDEX_BONE_WEIGHTS].offset =
                    offset_blendweights;
...
void R_AddIQMSurfaces(trRefEntity_t *ent) {
    ...
    for(i = 0; i < data->num_surfaces; i++) {
        ...
        if(!personalModel
            ...
            R_AddDrawSurf((surfaceType_t *)drawSurf, tr.shadowShader, 0, 0, 0, 0);
        }
        ...
        if(r_shadows->integer == 3
                ...
                && shader->sort == SS_OPAQUE) {
            R_AddDrawSurf((surfaceType_t *)drawSurf, tr.projectionShadowShader, 0, 0,
                          0, 0);
        }

        if(!personalModel) {
            R_AddDrawSurf((surfaceType_t *)drawSurf, shader, fogNum, 0, 0,
                          cubemapIndex);
        }
...
static void ComputePoseMats(iqmData_t *data, sint frame, sint oldframe,
    ...
    if(oldframe == frame) {
        ...
        for(i = 0; i < data->num_poses; i++, oldpose++, pose++, relativeJoint++) {
            relativeJoint->translate[0] = oldpose->translate[0] * backlerp +
                                          pose->translate[0] * lerp;
            relativeJoint->translate[1] = oldpose->translate[1] * backlerp +
                                          pose->translate[1] * lerp;
            relativeJoint->translate[2] = oldpose->translate[2] * backlerp +
                                          pose->translate[2] * lerp;

            relativeJoint->scale[0] = oldpose->scale[0] * backlerp + pose->scale[0] *
                                      lerp;
            relativeJoint->scale[1] = oldpose->scale[1] * backlerp + pose->scale[1] *
                                      lerp;
            relativeJoint->scale[2] = oldpose->scale[2] * backlerp + pose->scale[2] *
                                      lerp;
...
void RB_IQMSurfaceAnim(surfaceType_t *surface) {
    ...
    sint    frame = data->num_frames ? backEnd.currentEntity->e.frame %
                    data->num_frames : 0;
    sint    oldframe = data->num_frames ? backEnd.currentEntity->e.oldframe %
                       data->num_frames : 0;
    ...
    if(data->num_poses > 0) {
        ...
        for(i = 0; i < surf->num_influences; i++) {
            ...
            if(data->blendWeightsType == IQM_FLOAT) {
                ...
                blendWeights[0] = static_cast<float32>(data->influenceBlendWeights.b[4 *
                                                       influence + 0]) / 255.0f;
                blendWeights[1] = static_cast<float32>(data->influenceBlendWeights.b[4 *
                                                       influence + 1]) / 255.0f;
                blendWeights[2] = static_cast<float32>(data->influenceBlendWeights.b[4 *
                                                       influence + 2]) / 255.0f;
                blendWeights[3] = static_cast<float32>(data->influenceBlendWeights.b[4 *
                                                       influence + 3]) / 255.0f;
            }
            ...
            if(blendWeights[0] <= 0.0f) {
                ...
                // four blend weights
                vtxMat[0] = blendWeights[0] * poseMats[12 * data->influenceBlendIndexes[4 *
                                                       influence + 0] + 0];
                vtxMat[1] = blendWeights[0] * poseMats[12 * data->influenceBlendIndexes[4 *
                                                       influence + 0] + 1];
                vtxMat[2] = blendWeights[0] * poseMats[12 * data->influenceBlendIndexes[4 *
                                                       influence + 0] + 2];
                vtxMat[3] = blendWeights[0] * poseMats[12 * data->influenceBlendIndexes[4 *
                                                       influence + 0] + 3];
                vtxMat[4] = blendWeights[0] * poseMats[12 * data->influenceBlendIndexes[4 *
                                                       influence + 0] + 4];
                vtxMat[5] = blendWeights[0] * poseMats[12 * data->influenceBlendIndexes[4 *
                                                       influence + 0] + 5];
                vtxMat[6] = blendWeights[0] * poseMats[12 * data->influenceBlendIndexes[4 *
                                                       influence + 0] + 6];
                vtxMat[7] = blendWeights[0] * poseMats[12 * data->influenceBlendIndexes[4 *
                                                       influence + 0] + 7];
                vtxMat[8] = blendWeights[0] * poseMats[12 * data->influenceBlendIndexes[4 *
                                                       influence + 0] + 8];
                vtxMat[9] = blendWeights[0] * poseMats[12 * data->influenceBlendIndexes[4 *
                                                       influence + 0] + 9];
                vtxMat[10] = blendWeights[0] * poseMats[12 * data->influenceBlendIndexes[4
                                                        * influence + 0] + 10];
                vtxMat[11] = blendWeights[0] * poseMats[12 * data->influenceBlendIndexes[4
                                                        * influence + 0] + 11];
                ...
                for(j = 1; j < 3; j++) {
                    ...
                    vtxMat[0] += blendWeights[j] * poseMats[12 * data->influenceBlendIndexes[4
                                                            * influence + j] + 0];
                    vtxMat[1] += blendWeights[j] * poseMats[12 * data->influenceBlendIndexes[4
                                                            * influence + j] + 1];
                    vtxMat[2] += blendWeights[j] * poseMats[12 * data->influenceBlendIndexes[4
                                                            * influence + j] + 2];
                    vtxMat[3] += blendWeights[j] * poseMats[12 * data->influenceBlendIndexes[4
                                                            * influence + j] + 3];
                    vtxMat[4] += blendWeights[j] * poseMats[12 * data->influenceBlendIndexes[4
                                                            * influence + j] + 4];
                    vtxMat[5] += blendWeights[j] * poseMats[12 * data->influenceBlendIndexes[4
                                                            * influence + j] + 5];
                    vtxMat[6] += blendWeights[j] * poseMats[12 * data->influenceBlendIndexes[4
                                                            * influence + j] + 6];
                    vtxMat[7] += blendWeights[j] * poseMats[12 * data->influenceBlendIndexes[4
                                                            * influence + j] + 7];
                    vtxMat[8] += blendWeights[j] * poseMats[12 * data->influenceBlendIndexes[4
                                                            * influence + j] + 8];
                    vtxMat[9] += blendWeights[j] * poseMats[12 * data->influenceBlendIndexes[4
                                                            * influence + j] + 9];
                    vtxMat[10] += blendWeights[j] * poseMats[12 * data->influenceBlendIndexes[4
                                  * influence + j] + 10];
        ...
        for(i = 0; i < surf->num_vertexes;
            ...
            sint influence = data->influences[surf->first_vertex + i] -
...
void RB_IQMSurfaceAnimVao(srfVaoIQModel_t *surface) {
    ...
    if(glState.boneAnimation) {
        ...
        sint            frame = data->num_frames ? backEnd.currentEntity->e.frame %
                                data->num_frames : 0;
        sint            oldframe = data->num_frames ?
                                   backEnd.currentEntity->e.oldframe % data->num_frames : 0;
        float32     backlerp = backEnd.currentEntity->e.backlerp;
        sint i;
