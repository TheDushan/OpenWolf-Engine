...
void GL_State(uint32 stateBits) {
    ...
    if(diff & (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS)) {
        uint oldState = glState.glStateBits & (GLS_SRCBLEND_BITS |
                                               GLS_DSTBLEND_BITS);
        uint newState = stateBits & (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS);
        uint storedState = glState.storedGlState & (GLS_SRCBLEND_BITS |
                           GLS_DSTBLEND_BITS);
...
void RB_BeginDrawingView(void) {
    ...
    if(glRefConfig.framebufferObject) {
        ...
        if(tr.renderCubeFbo && fbo == tr.renderCubeFbo) {
            ...
            FBO_AttachImage(fbo, cubemap->image, GL_COLOR_ATTACHMENT0_EXT,
    ...
    if(backEnd.viewParms.isPortal) {
        ...
        plane2[3] = DotProduct(plane,
                               backEnd.viewParms.orientation.origin) - plane[3];
#endif
...
void RB_RenderDrawSurfList(drawSurf_t *drawSurfs, sint numDrawSurfs) {
    ...
    for(i = 0, drawSurf = drawSurfs ; i < numDrawSurfs ; i++, drawSurf++) {
        ...
        if(shader != nullptr && (shader != oldShader || fogNum != oldFogNum ||
                                 dlighted != oldDlighted || pshadowed != oldPshadowed ||
        ...
        if(entityNum != oldEntityNum) {
            ...
            if(entityNum != REFENTITYNUM_WORLD) {
                ...
                backEnd.refdef.floatTime = originalTime - static_cast<float64>
...
void    RB_SetGL2D(void) {
    ...
    Mat4Ortho(0, static_cast<float32>(width), static_cast<float32>(height), 0,
              0, 1, matrix);
...
=============
*/
void idRenderSystemLocal::DrawStretchRaw(sint x, sint y, sint w, sint h,
        sint cols, sint rows, const uchar8 *data, sint client, bool dirty) {
    sint            i, j;
    sint            start, end;
    vec4_t quadVerts[4];
    vec2_t texCoords[4];
    ...
    VectorSet4(quadVerts[0], static_cast<float32>(x), static_cast<float32>(y),
               0.0f, 1.0f);
    VectorSet4(quadVerts[1], static_cast<float32>(x + w),
               static_cast<float32>(y),     0.0f, 1.0f);
    VectorSet4(quadVerts[2], static_cast<float32>(x + w),
               static_cast<float32>(y + h), 0.0f, 1.0f);
    VectorSet4(quadVerts[3], static_cast<float32>(x),
               static_cast<float32>(y + h), 0.0f, 1.0f);
...
const void *RB_UploadCinematics(const void *data) {
    ...
    if(cmd->cols != tr.scratchImage[client]->width ||
        ...
        qglTextureParameterfEXT(texture, GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
                                GL_LINEAR);
        qglTextureParameterfEXT(texture, GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                                GL_CLAMP_TO_EDGE);
...
static const void *RB_PrefilterEnvMap(const void *data) {
    const convolveCubemapCommand_t *cmd = (const convolveCubemapCommand_t *)
                                          data;
    ...
    for(sint level = 1; level <= r_cubemapSize->integer; level++) {
        ...
        for(sint cubemapSide = 0; cubemapSide < 6; cubemapSide++) {
            vec4_t viewInfo;
            VectorSet4(viewInfo, static_cast<float32>(cubemapSide), level,
                       r_cubemapSize->integer,
                       static_cast<float32>(level) / static_cast<float32>
                       (r_cubemapSize->integer));
            GLSL_SetUniformVec4(&tr.prefilterEnvMapShader, UNIFORM_VIEWINFO, viewInfo);
            RB_InstantQuad2(quadVerts, texCoords);
            qglCopyTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + cubemapSide, level,
...
const void *RB_DrawSurfs(const void *data) {
    ...
    if(glRefConfig.framebufferObject &&
            (backEnd.viewParms.flags & VPF_DEPTHCLAMP) && glRefConfig.depthClamp) {
        qglEnable(GL_DEPTH_CLAMP);
    }

    if(glRefConfig.framebufferObject &&
            !(backEnd.refdef.rdflags & RDF_NOWORLDMODEL) && (r_depthPrepass->integer ||
                    isShadowView)) {
        FBO_t *oldFbo = glState.currentFBO;
        vec4_t viewInfo;

        VectorSet4(viewInfo, backEnd.viewParms.zFar / r_znear->value,
                   backEnd.viewParms.zFar, 0.0, 0.0);
        ...
        if(!isShadowView) {
            if(tr.renderFbo == nullptr && tr.renderDepthImage) {
                ...
                if(!glRefConfig.intelGraphics) {
                    qglCopyTextureSubImage2DEXT(tr.renderDepthImage->texnum, GL_TEXTURE_2D, 0,
                                                0, 0, 0, 0, glConfig.vidWidth, glConfig.vidHeight);
                }
            }
            ...
            if(tr.hdrDepthFbo) {
                ...
                FBO_BlitFromTexture(tr.renderDepthImage, srcTexCoords, nullptr,
                                    tr.hdrDepthFbo, nullptr, nullptr, nullptr, 0);
            }
            ...
            if(r_sunlightMode->integer && backEnd.viewParms.flags & VPF_USESUNLIGHT) {
                ...
                box[0] = backEnd.viewParms.viewportX * tr.screenShadowFbo->width /
                         static_cast<float32>(glConfig.vidWidth);
                box[1] = backEnd.viewParms.viewportY * tr.screenShadowFbo->height /
                         static_cast<float32>(glConfig.vidHeight);
                box[2] = backEnd.viewParms.viewportWidth * tr.screenShadowFbo->width /
                         static_cast<float32>(glConfig.vidWidth);
                box[3] = backEnd.viewParms.viewportHeight * tr.screenShadowFbo->height /
                         static_cast<float32>(glConfig.vidHeight);

                qglViewport(static_cast<sint>(box[0]), static_cast<sint>(box[1]),
                            static_cast<sint>(box[2]), static_cast<sint>(box[3]));
                qglScissor(static_cast<sint>(box[0]), static_cast<sint>(box[1]),
                           static_cast<sint>(box[2]), static_cast<sint>(box[3]));

                box[0] = backEnd.viewParms.viewportX / static_cast<float32>
                         (glConfig.vidWidth);
                box[1] = backEnd.viewParms.viewportY / static_cast<float32>
                         (glConfig.vidHeight);
                box[2] = box[0] + backEnd.viewParms.viewportWidth / static_cast<float32>
                         (glConfig.vidWidth);
                box[3] = box[1] + backEnd.viewParms.viewportHeight / static_cast<float32>
                         (glConfig.vidHeight);
                ...
                GLSL_SetUniformVec3(&tr.shadowmaskShader, UNIFORM_VIEWORIGIN,
                ...
                RB_InstantQuad2(quadVerts,
                                texCoords);   //, color, shaderProgram, invTexRes);
            ...
            if(r_ssao->integer) {
                ...
                viewInfo[2] = 1.0f / (static_cast<float32>((tr.quarterImage[0]->width)) *
                                      tanf(backEnd.viewParms.fovX * M_PI / 360.0f) * 2.0f);
                viewInfo[3] = 1.0f / (static_cast<float32>((tr.quarterImage[0]->height)) *
                                      tanf(backEnd.viewParms.fovY * M_PI / 360.0f) * 2.0f);
                viewInfo[3] *= static_cast<float32>(backEnd.viewParms.viewportHeight) /
                               static_cast<float32>(backEnd.viewParms.viewportWidth);
    ...
    if(glRefConfig.framebufferObject && (tr.renderCubeFbo != nullptr &&
        ...
        if(cmd->cubeSide != -1) {
            if(tr.shadowCubemaps[cmd->map]) {
                qglCopyTextureSubImage2DEXT(tr.shadowCubemaps[cmd->map]->texnum,
                                            GL_TEXTURE_CUBE_MAP_POSITIVE_X + cmd->cubeSide, 0, 0, 0, backEnd.refdef.x,
                                            glConfig.vidHeight - (backEnd.refdef.y + PSHADOW_MAP_SIZE),
                                            PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE);
            }
        } else {
            if(tr.pshadowMaps[cmd->map]) {
                qglCopyTextureSubImage2DEXT(tr.pshadowMaps[cmd->map]->texnum,
                                            GL_TEXTURE_2D, 0, 0, 0, backEnd.refdef.x,
                                            glConfig.vidHeight - (backEnd.refdef.y + PSHADOW_MAP_SIZE),
                                            PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE);
            }
        }
...
const void *RB_PostProcess(const void *data) {
    ...
    if(r_ssao->integer) {
        srcBox[0] = backEnd.viewParms.viewportX * tr.screenSsaoImage->width /
                    glConfig.vidWidth;
        srcBox[1] = backEnd.viewParms.viewportY * tr.screenSsaoImage->height /
                    glConfig.vidHeight;
        srcBox[2] = backEnd.viewParms.viewportWidth * tr.screenSsaoImage->width /
                    glConfig.vidWidth;
        srcBox[3] = backEnd.viewParms.viewportHeight * tr.screenSsaoImage->height /
                    glConfig.vidHeight;

        FBO_Blit(tr.screenSsaoFbo, srcBox, nullptr, srcFbo, dstBox, nullptr,
                 nullptr, GLS_SRCBLEND_DST_COLOR | GLS_DSTBLEND_ZERO);
    }
    ...
    if(srcFbo) {
        if(backEnd.refdef.rdflags & RDF_UNDERWATER) {
            ...
            FBO_FastBlit(tr.genericFbo, srcBox, srcFbo, dstBox, GL_COLOR_BUFFER_BIT,
                         GL_NEAREST);
        }
        ...
        if(r_hdr->integer && (r_toneMap->integer || r_forceToneMap->integer)) {
            ...
            FBO_FastBlit(srcFbo, srcBox, nullptr, dstBox, GL_COLOR_BUFFER_BIT,
                         GL_NEAREST);
        } else {
            vec4_t color;
    ...
    if(0) {
        ...
        iQtrBox[0] = backEnd.viewParms.viewportX      * tr.quarterImage[0]->width /
                     static_cast<float32>(glConfig.vidWidth);
        iQtrBox[1] = backEnd.viewParms.viewportY      *
                     tr.quarterImage[0]->height / static_cast<float32>(glConfig.vidHeight);
        iQtrBox[2] = backEnd.viewParms.viewportWidth  * tr.quarterImage[0]->width /
                     static_cast<float32>(glConfig.vidWidth);
        iQtrBox[3] = backEnd.viewParms.viewportHeight *
                     tr.quarterImage[0]->height / static_cast<float32>(glConfig.vidHeight);
        ...
        FBO_FastBlit(tr.quarterFbo[1], nullptr, nullptr, nullptr,
                     GL_COLOR_BUFFER_BIT, GL_LINEAR);
        FBO_Bind(nullptr);
    }
    ...
    if(0 && r_sunlightMode->integer) {
        ...
        FBO_BlitFromTexture(tr.sunShadowDepthImage[0], nullptr, nullptr, nullptr,
                            dstBox, nullptr, nullptr, 0);
        VectorSet4(dstBox, 128, glConfig.vidHeight - 128, 128, 128);
        FBO_BlitFromTexture(tr.sunShadowDepthImage[1], nullptr, nullptr, nullptr,
                            dstBox, nullptr, nullptr, 0);
        VectorSet4(dstBox, 256, glConfig.vidHeight - 128, 128, 128);
        FBO_BlitFromTexture(tr.sunShadowDepthImage[2], nullptr, nullptr, nullptr,
                            dstBox, nullptr, nullptr, 0);
        VectorSet4(dstBox, 384, glConfig.vidHeight - 128, 128, 128);
        FBO_BlitFromTexture(tr.sunShadowDepthImage[3], nullptr, nullptr, nullptr,
                            dstBox, nullptr, nullptr, 0);
    }
    ...
    if(0 && r_shadows->integer == 4) {
        ...
        FBO_BlitFromTexture(tr.pshadowMaps[0], nullptr, nullptr, nullptr, dstBox,
                            nullptr, nullptr, 0);
        VectorSet4(dstBox, 512 + 128, glConfig.vidHeight - 128, 128, 128);
        FBO_BlitFromTexture(tr.pshadowMaps[1], nullptr, nullptr, nullptr, dstBox,
                            nullptr, nullptr, 0);
        VectorSet4(dstBox, 512 + 256, glConfig.vidHeight - 128, 128, 128);
        FBO_BlitFromTexture(tr.pshadowMaps[2], nullptr, nullptr, nullptr, dstBox,
                            nullptr, nullptr, 0);
        VectorSet4(dstBox, 512 + 384, glConfig.vidHeight - 128, 128, 128);
        FBO_BlitFromTexture(tr.pshadowMaps[3], nullptr, nullptr, nullptr, dstBox,
                            nullptr, nullptr, 0);
    }
    ...
    if(0) {
        ...
        FBO_BlitFromTexture(tr.renderDepthImage, nullptr, nullptr, nullptr, dstBox,
                            nullptr, nullptr, 0);
        VectorSet4(dstBox, 512, glConfig.vidHeight - 256, 256, 256);
        FBO_BlitFromTexture(tr.screenShadowImage, nullptr, nullptr, nullptr,
                            dstBox, nullptr, nullptr, 0);
    }
    ...
    if(0) {
        ivec4_t dstBox;
        VectorSet4(dstBox, 256, glConfig.vidHeight - 256, 256, 256);
        FBO_BlitFromTexture(tr.sunRaysImage, nullptr, nullptr, nullptr, dstBox,
                            nullptr, nullptr, 0);
    }
    ...
    if(r_cubeMapping->integer && tr.numCubemaps) {
        ivec4_t dstBox;
        sint cubemapIndex = R_CubemapForPoint(
        ...
        if(cubemapIndex) {
            ...
            FBO_BlitFromTexture(tr.cubemaps[cubemapIndex - 1].image, nullptr, nullptr,
                                nullptr, dstBox, &tr.testcubeShader, nullptr, 0);
        }
    }
...
const void *RB_ExportCubemaps(const void *data) {
    ...
    if(cmd) {
        ...
        for(i = 0; i < tr.numCubemaps; i++) {
            ...
            for(j = 0; j < 6; j++) {
                FBO_AttachImage(tr.renderCubeFbo, cubemap->image, GL_COLOR_ATTACHMENT0_EXT,
                                j);
                qglReadPixels(0, 0, r_cubemapSize->integer, r_cubemapSize->integer,
