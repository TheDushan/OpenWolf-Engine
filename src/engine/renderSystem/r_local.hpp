...
#ifndef __R_LOCAL_HPP__
#define __R_LOCAL_HPP__
...
typedef struct dlight_s {
    ...
    sint
    additive;           // texture detail is lost tho when the lightmap is dark
} dlight_t;
...
typedef struct {
    ...
    vec3_t
    lightDir;       // normalized direction towards light, in world space
    vec3_t
    modelLightDir;  // normalized direction towards light, in model space
...
typedef struct vao_s {
    ...
    uint        vertexesVBO;
    sint
    vertexesSize;  // amount of memory data allocated for all vertices in bytes
    vaoAttrib_t     attribs[VAO_MAX_ATTRIBS];

    uint
    frameSize;      // bytes to skip per frame when doing vertex animation

    uint        indexesIBO;
    sint
    indexesSize;   // amount of memory data allocated for all triangles in bytes
} vao_t;
...
typedef struct {
    ...
    float32
    matrix[2][2];       // s' = s * m[0][0] + t * m[1][0] + trans[0]
    float32
    translate[2];       // t' = s * m[0][1] + t * m[0][1] + trans[1]
...
typedef struct {
    ...
    uchar8
    constantColor[4];           // for CGEN_CONST and AGEN_CONST
...
typedef struct shader_s {
    ...
    sint
    lightmapIndex;          // for a shader to match, both name and lightmapIndex must match

    sint            index;                  // this shader == tr.shaders[index]
    sint
    sortedIndex;            // this shader == tr.sortedShaders[sortedIndex]
    ...
    cullType_t
    cullType;               // CT_FRONT_SIDED, CT_BACK_SIDED, or CT_TWO_SIDED
    ...
    fogPass_t
    fogPass;                // draw a blended pass, possibly with depth test equals
    ...
    struct shader_s
        *remappedShader;                  // current shader this one is remapped too
...
typedef struct {
    ...
    sint
    time;               // time in milliseconds for shader effects and other time dependent rendering issues
...
typedef struct skin_s {
    ...
    sint            numSurfaces;
    skinSurface_t
    *surfaces;           // dynamically allocated array of surfaces
} skin_t;
...
typedef struct {
    ...
    vec3_t
    pvsOrigin;          // may be different than or.origin for portals
...
typedef struct mnode_s {
    ...
    sint
    contents;       // -1 for nodes, to differentiate from leafs
    sint
    visCounts[MAX_VISCOUNTS];  // node needs to be traversed if current
...
typedef struct {
    ...
    sint            clusterBytes;
    const uchar8
    *vis;            // may be passed in by CM_LoadMap to save space
...
*/
typedef struct {
    sint        c_sphere_cull_patch_in, c_sphere_cull_patch_clip,
                c_sphere_cull_patch_out;
    sint        c_box_cull_patch_in, c_box_cull_patch_clip,
                c_box_cull_patch_out;
    sint        c_sphere_cull_md3_in, c_sphere_cull_md3_clip,
...
typedef struct {
    ...
    uint
    vertexAttribsEnabled;  // global if no VAOs, tess only otherwise
    FBO_t          *currentFBO;
    vao_t          *currentVao;
    mat4_t        modelview;
    mat4_t        projection;
...
void R_AddDrawSurf(surfaceType_t *surface, shader_t *shader, sint fogIndex,
                   sint dlightMap, sint pshadowMap, sint cubemap);
void R_CalcTexDirs(vec3_t sdir, vec3_t tdir, const vec3_t v1,
                   const vec3_t v2, const vec3_t v3, const vec2_t w1, const vec2_t w2,
                   const vec2_t w3);
...
sint R_CullPointAndRadiusEx(const vec3_t origin, float32 radius,
...
void R_RotateForEntity(const trRefEntity_t *ent,
                       const viewParms_t *viewParms, orientationr_t *orientation);
...
void R_GridInsertColumn(srfBspSurface_t *grid, sint column, sint row,
                        vec3_t point, float32 loderror);
void R_GridInsertRow(srfBspSurface_t *grid, sint row, sint column,
                     vec3_t point, float32 loderror);
...
vao_t *R_CreateVao2(pointer name, sint numVertexes, srfVert_t *verts,
                    sint numIndexes, uint *inIndexes);
...
sint R_IQMLerpTag(orientation_t *tag, iqmData_t *data, sint startFrame,
...
=============================================================
*/
void    R_TransformModelToClip(const vec3_t src,
                               const float32 *modelMatrix, const float32 *projectionMatrix,
...
void    RB_CalcScrollTexMatrix(const float32 scrollSpeed[2],
                               float32 *matrix);
void    RB_CalcRotateTexMatrix(float32 degsPerSecond, float32 *matrix);
void RB_CalcTurbulentFactors(const waveForm_t *wf, float32 *amplitude,
                             float32 *now);
void    RB_CalcTransformTexMatrix(const texModInfo_t *tmi,
                                  float32 *matrix);
...
void RE_SaveJPG(valueType *filename, sint quality, sint image_width,
                sint image_height, uchar8 *image_buffer, sint padding);
uint32 RE_SaveJPGToBuffer(uchar8 *buffer, uint32 bufSize, sint quality,
                          sint image_width, sint image_height, uchar8 *image_buffer, sint padding);
...
public:
    ...
    virtual void AddPolyToScene(qhandle_t hShader, sint numVerts,
                                const polyVert_t *verts, sint num);
    ...
    virtual void UploadCinematic(sint w, sint h, sint cols, sint rows,
                                 const uchar8 *data, sint client, bool dirty);
