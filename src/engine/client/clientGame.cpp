...
====================
*/
void idClientGameSystemLocal::GetCurrentSnapshotNumber(
    sint *snapshotNumber, sint *serverTime) {
...
====================
*/
bool idClientGameSystemLocal::GetSnapshot(sint snapshotNumber,
        snapshot_t *snapshot) {
    sint i, count;
    clSnapshot_t *clSnap;
    ...
    for(i = 0; i < count; i++) {
        snapshot->entities[i] = cl.parseEntities[(clSnap->parseEntitiesNum + i) &
...
==============
*/
void idClientGameSystemLocal::SetUserCmdValue(sint userCmdValue,
        sint flags, float32 sensitivityScale, sint mpIdentClient) {
...
==================
*/
void idClientGameSystemLocal::SetClientLerpOrigin(float32 x, float32 y,
        float32 z) {
...
=====================
*/
void idClientGameSystemLocal::CompleteCgameCommand(valueType *args,
        sint argNum) {
...
=====================
*/
void idClientGameSystemLocal::CgameCompletion(void(*callback)(pointer s),
        sint argNum) {
    completer = callback;
...
==============
*/
void idClientGameSystemLocal::AddCgameCommand(pointer cmdName,
        pointer cmdDesc) {
...
rescan:
    ...
    if(!strcmp(cmd, "bcs0")) {
        Q_vsprintf_s(bigConfigString, BIG_INFO_STRING, BIG_INFO_STRING,
                     "cs %s \"%s", cmdSystem->Argv(1), cmdSystem->Argv(2));
        return false;
    }
...
void idClientGameSystemLocal::ShutdownCGame(void) {
    clientGUISystem->SetCatcher(clientGUISystem->GetCatcher() &
                                ~KEYCATCH_CGAME);
...
void idClientGameSystemLocal::KeySetCatcher(sint catcher) {
    clientGUISystem->SetCatcher(catcher | (clientGUISystem->GetCatcher() &
                                           KEYCATCH_CONSOLE));
}
...
void idClientGameSystemLocal::InitCGame(void) {
    ...
    Q_vsprintf_s(cl.mapname, sizeof(cl.mapname), sizeof(cl.mapname),
                 "maps/%s.bsp", mapname);
    ...
    cgameEntry = (idCGame * (QDECL *)(cgameImports_t *))
...
void idClientGameSystemLocal::AdjustTimeDelta(void) {
    ...
    if(deltaDelta > RESET_TIME) {
        ...
        cl.oldServerTime =
            cl.snapServer.serverTime;    // FIXME: is this a problem for cgame?
...
void idClientGameSystemLocal::SetCGameTime(void) {
    ...
    if(clc.demoplaying && cl_freezeDemo->integer) {
        ...
        if(timeNudge < 0 && (cl.snapServer.ps.pm_type == PM_SPECTATOR ||
                             cl.snapServer.ps.pm_flags & PMF_FOLLOW || clc.demoplaying)) {
...
====================
*/
bool idClientGameSystemLocal::GetTag(sint clientNum, valueType *tagname,
                                     orientation_t *_or) {
    if(!cgvm) {
        return false;
    }
