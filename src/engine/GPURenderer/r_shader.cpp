...
void idRenderSystemLocal::RemapShader(pointer shaderName,
                                      pointer newShaderName, pointer timeOffset) {
...
static bool ParseStage(shaderStage_t *stage, valueType **text) {
    valueType *token;
    sint depthMaskBits = GLS_DEPTHMASK_TRUE, blendSrcBits = 0,
    ...
    while(1) {
        ...
        //
            ...
            while(1) {
                ...
                if(num < MAX_IMAGE_ANIMATIONS) {
                    ...
                    stage->bundle[0].image[num] = R_FindImageFile(token, IMGTYPE_COLORALPHA,
                                                  flags);
        ...
        //
            ...
            if(!Q_stricmp(token, "diffuseMap")) {
                ...
                VectorSet4(stage->normalScale, r_baseNormalX->value, r_baseNormalY->value,
                           1.0f, r_baseParallax->value);
    ...
    if(stage->alphaGen == AGEN_IDENTITY) {
        if(stage->rgbGen ==
                CGEN_IDENTITY /* || stage->rgbGen == CGEN_LIGHTING_DIFFUSE*/) {
...
static void ParseSkyParms(valueType **text) {
    ...
    if(strcmp(token, "-")) {
        for(i = 0 ; i < 6 ; i++) {
            Q_vsprintf_s(pathname, sizeof(pathname), sizeof(pathname), "%s_%s.tga",
                         token, suf[i]);
            shader.sky.outerbox[i] = R_FindImageFile(const_cast< valueType * >
                                     (pathname), IMGTYPE_COLORALPHA, imgFlags | IMGFLAG_CLAMPTOEDGE);
    ...
    if(strcmp(token, "-")) {
        for(i = 0 ; i < 6 ; i++) {
            Q_vsprintf_s(pathname, sizeof(pathname), sizeof(pathname), "%s_%s.tga",
                         token, suf[i]);
            shader.sky.innerbox[i] = R_FindImageFile(const_cast< valueType * >
                                     (pathname), IMGTYPE_COLORALPHA, imgFlags);
...
static bool StringsContainWord(pointer heystack, pointer heystack2,
                               valueType *needle) {
...
static bool ParseShader(pointer name, valueType **text) {
    ...
    while(1) {
        ...
        // material undeprecated as of 7 May 01 - q3map_material deprecated
                ...
                !Q_stricmp(token, "q3map_material")) {
            ParseMaterial(text);
            continue;
        }
        ...
        // sun parms
                ...
                !Q_stricmp(token, "q3map_sunExt") || !Q_stricmp(token, "q3gl2_sun")) {
            float32 a, b;
            bool isGL2Sun = false;
        ...
        // q3map_surfacelight deprecated as of 16 Jul 01
                ...
                !Q_stricmp(token, "q3map_surfacelight")) {
            SkipRestOfLine(text);
            continue;
        ...
        // fogParms
            ...
            if(r_greyscale->integer) {
                ...
                luminance = LUMA(shader.fogParms.color[0], shader.fogParms.color[1],
                ...
                luminance = LUMA(shader.fogParms.color[0], shader.fogParms.color[1],
                                 shader.fogParms.color[2]);
                shader.fogParms.color[0] = LERP(shader.fogParms.color[0], luminance,
                                                r_greyscale->value);
                shader.fogParms.color[1] = LERP(shader.fogParms.color[1], luminance,
                                                r_greyscale->value);
                shader.fogParms.color[2] = LERP(shader.fogParms.color[2], luminance,
                                                r_greyscale->value);
            }
    ...
    if(!HaveSurfaceType(shader.surfaceFlags)) {
        if(StringsContainWord(name, name, "plastic") ||
                StringsContainWord(name, name, "trooper") ||
                StringsContainWord(name, name, "medpack") ||
                StringsContainWord(name, name, "reborn")) {
            shader.surfaceFlags |= MATERIAL_PLASTIC;
                  ...
                  StringsContainWord(name, name, "blaster") ||
                  StringsContainWord(name, name, "pistol") ||
                  StringsContainWord(name, name, "thermal") ||
                  StringsContainWord(name, name, "bowcaster") ||
                  StringsContainWord(name, name, "cannon") ||
                  StringsContainWord(name, name, "saber") ||
                  StringsContainWord(name, name, "rifle") ||
                  StringsContainWord(name, name, "jetpack")) {
            ...
            shader.surfaceFlags |= MATERIAL_CONCRETE;
                  ...
                  StringsContainWord(name, name, "teeth") ||
                  StringsContainWord(name, name, "stucco")) {
            ...
            shader.surfaceFlags |= MATERIAL_FABRIC;
                  ...
                  StringsContainWord(name, name, "robe") ||
    ...
    if(StringsContainWord(name, *text, "plastic") ||
            StringsContainWord(name, *text, "trooper") ||
            StringsContainWord(name, *text, "medpack") ||
            StringsContainWord(name, *text, "reborn"))
        if(!(shader.surfaceFlags & MATERIAL_PLASTIC)) {
            shader.surfaceFlags |= MATERIAL_PLASTIC;
        }
...
static void CollapseStagesToLightall(shaderStage_t *diffuse,
    ...
    if(r_normalMapping->integer) {
        ...
        if(normal) {
            ...
            image_t *normalImg;
            sint/*imgFlags_t*/ normalFlags = (diffuseImg->flags &
            ...
            if(normalImg) {
                ...
                VectorSet4(diffuse->normalScale, r_baseNormalX->value,
                           r_baseNormalY->value, 1.0f, r_baseParallax->value);
            }
        }
    }
    ...
    if(r_specularMapping->integer) {
        ...
        if(specular) {
            ...
            VectorCopy4(specular->specularScale, diffuse->specularScale);
                  ...
                  (diffuseImg = diffuse->bundle[TB_DIFFUSEMAP].image[0]) && r_pbr->integer) {
            valueType specularName[MAX_QPATH];
            image_t *specularImg;
            sint/*imgFlags_t*/ specularFlags = (diffuseImg->flags &
                                                ~IMGFLAG_GENNORMALMAP) | IMGFLAG_NOLIGHTSCALE;

            COM_StripExtension2(diffuseImg->imgName, specularName, MAX_QPATH);
            Q_strcat(specularName, MAX_QPATH, "_s");

            specularImg = R_FindImageFile(specularName, IMGTYPE_COLORALPHA,
                                          specularFlags);
...
static sint CollapseStagesToGLSL(void) {
    ...
    if(!skip) {
        ...
        if(stages[0].active && stages[0].bundle[0].tcGen == TCGEN_LIGHTMAP &&
                stages[1].active) {
            sint blendBits = stages[1].stateBits & (GLS_DSTBLEND_BITS |
                                                    GLS_SRCBLEND_BITS);
    ...
    if(!skip) {
        ...
        for(i = 0; i < MAX_SHADER_STAGES; i++) {
            ...
            if(pStage->bundle[0].tcGen == TCGEN_LIGHTMAP) {
                sint blendBits = pStage->stateBits & (GLS_DSTBLEND_BITS |
                                                      GLS_SRCBLEND_BITS);
    ...
    if(!skip) {
        for(i = 0; i < MAX_SHADER_STAGES; i++) {
            ...
            if(diffuse->rgbGen == CGEN_VERTEX_LIT ||
                    diffuse->rgbGen == CGEN_EXACT_VERTEX_LIT) {
                vertexlit = true;
            }

            CollapseStagesToLightall(diffuse, normal, specular, lightmap, diffuselit,
                                     vertexlit, parallax, tcgen);
        }
                    ...
                    for(i = 0, drawSurf = ds_cmd->drawSurfs; i < ds_cmd->numDrawSurfs;
                        ...
                        if(sortedIndex >= newShader) {
                            ...
                            drawSurf->sort = (sortedIndex << QSORT_SHADERNUM_SHIFT) | entityNum |
                                             (fogNum << QSORT_FOGNUM_SHIFT) | (static_cast<sint>(pshadowMap) <<
                                                     QSORT_PSHADOW_SHIFT) | static_cast<sint>(dlightMap);
                        }
                    }
...
static void FindLightingStages(void) {
    ...
    if(shader.isSky || (shader.surfaceFlags & (SURF_NODLIGHT | SURF_SKY)) ||
            shader.sort > SS_OPAQUE) {
        return;
    }
    ...
    for(i = 0; i < shader.numUnfoggedPasses; i++) {
        if(!stages[i].bundle[0].isLightmap) {
            ...
            if((stages[i].stateBits & GLS_BLEND_BITS) == (GLS_SRCBLEND_ONE |
                    GLS_DSTBLEND_ONE)) {
                continue;
            }

            if(stages[i].rgbGen == CGEN_IDENTITY &&
                    (stages[i].stateBits & GLS_BLEND_BITS) == (GLS_SRCBLEND_DST_COLOR |
                            GLS_DSTBLEND_ZERO)) {
...
===============
*/
shader_t *R_FindShader(pointer name, sint lightmapIndex,
                       bool mipRawImage) {
    valueType       strippedName[MAX_QPATH];
    sint            hash;
    ...
    if(shader.lightmapIndex == LIGHTMAP_NONE) {
        ...
        if(implicitStateBits & (GLS_ATEST_BITS | GLS_SRCBLEND_BITS |
                                GLS_DSTBLEND_BITS)) {
            stages[0].bundle[0].image[0] = image;
            stages[0].active = true;
            ...
            stages[1].active = true;
            stages[1].rgbGen = CGEN_IDENTITY;
            stages[1].stateBits = GLS_DEFAULT | GLS_SRCBLEND_DST_COLOR |
                                  GLS_DSTBLEND_ZERO | GLS_DEPTHFUNC_EQUAL;
        } else {
...
static void CreateExternalShaders(void) {
    tr.projectionShadowShader = R_FindShader("projectionShadow", LIGHTMAP_NONE,
                                true);
    ...
    if(tr.sunFlareShader->defaultShader) {
        ...
        if(!tr.flareShader->defaultShader && tr.flareShader->stages[0] &&
...
void R_InitShaders(void) {
    ...
    ::memset(shaderTextHashTable, 0,
             sizeof(shaderTextHashTable));     // drakkar - clear shader hashtable
...
void R_InitExternalShaders(void) {
    ...
    tr.sunShader = R_FindShader(tr.sunShaderName, LIGHTMAP_NONE,
                                MIP_RAW_IMAGE);
}