...
static sint R_MDRCullModel(mdrHeader_t *header, trRefEntity_t *ent) {
    ...
    newFrame = (mdrFrame_t *)(reinterpret_cast<uchar8 *>(header +
                              header->ofsFrames + frameSize * ent->e.frame));
    oldFrame = (mdrFrame_t *)(reinterpret_cast<uchar8 *>(header +
                              header->ofsFrames + frameSize * ent->e.oldframe));
...
sint R_MDRComputeFogNum(mdrHeader_t *header, trRefEntity_t *ent) {
    ...
    mdrFrame = (mdrFrame_t *)(reinterpret_cast<uchar8 *>(header +
                              header->ofsFrames + frameSize * ent->e.frame));
...
void R_MDRAddAnimSurfaces(trRefEntity_t *ent) {
    ...
    surface = (mdrSurface_t *)(reinterpret_cast<uchar8 *>
                               (lod) + lod->ofsSurfaces);
    ...
    for(i = 0 ; i < lod->numSurfaces ; i++) {
        ...
        if(r_shadows->integer == 3
                ...
                && shader->sort == SS_OPAQUE) {
            R_AddDrawSurf((surfaceType_t *)surface, tr.projectionShadowShader, 0,
                          false, false, 0);
        }
...
void RB_MDRSurfaceAnim(mdrSurface_t *surface) {
    ...
    header = (mdrHeader_t *)(reinterpret_cast<uchar8 *>(surface) +
                             surface->ofsHeader);
    ...
    triangles   = reinterpret_cast<sint *>((reinterpret_cast<uchar8 *>
                                            (surface) + surface->ofsTriangles));
    ...
    if(!backlerp) {
        ...
        for(i = 0 ; i < header->numBones * 12 ; i++) {
            (reinterpret_cast<float32 *>(bonePtr))[i] = frontlerp *
                    (reinterpret_cast<float32 *>(frame->bones))[i] + backlerp *
                    (reinterpret_cast<float32 *>(oldFrame->bones))[i];
        }
    }
    ...
    v = (mdrVertex_t *)(reinterpret_cast<uchar8 *>(surface) +
                        surface->ofsVerts);
    ...
    for(j = 0; j < numVerts; j++) {
        ...
        for(k = 0 ; k < v->numWeights ; k++, w++) {
            ...
            tempVert[0] += w->boneWeight * (DotProduct(bone->matrix[0],
                                            w->offset) + bone->matrix[0][3]);
            tempVert[1] += w->boneWeight * (DotProduct(bone->matrix[1],
                                            w->offset) + bone->matrix[1][3]);
            tempVert[2] += w->boneWeight * (DotProduct(bone->matrix[2],
                                            w->offset) + bone->matrix[2][3]);
