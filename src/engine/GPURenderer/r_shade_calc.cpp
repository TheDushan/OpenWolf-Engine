...
void RB_CalcBulgeVertexes(deformStage_t *ds) {
    ...
    for(i = 0; i < tess.numVertexes; i++, xyz += 4, st += 2, normal += 4) {
        ...
        off = static_cast<float32>(FUNCTABLE_SIZE / (M_PI * 2)) *
...
void DeformText(pointer text) {
    ...
    for(i = 0 ; i < len ; i++) {
        ...
        if(ch != ' ') {
            ...
            RB_AddQuadStampExt(origin, width, height, color, fcol, frow, fcol + size,
                               frow + size);
        }
...
static void AutospriteDeform(void) {
    ...
    if(tess.numVertexes & 3) {
        CL_RefPrintf(PRINT_WARNING, "Autosprite shader %s had odd vertex count\n",
                     tess.shader->name);
    }

    if(tess.numIndexes != (tess.numVertexes >> 2) * 6) {
        CL_RefPrintf(PRINT_WARNING, "Autosprite shader %s had odd index count\n",
                     tess.shader->name);
    }
...
static void Autosprite2Deform(void) {
    ...
    if(tess.numVertexes & 3) {
        CL_RefPrintf(PRINT_WARNING, "Autosprite2 shader %s had odd vertex count\n",
                     tess.shader->name);
    }

    if(tess.numIndexes != (tess.numVertexes >> 2) * 6) {
        CL_RefPrintf(PRINT_WARNING, "Autosprite2 shader %s had odd index count\n",
                     tess.shader->name);
    }
...
float32 RB_CalcWaveColorSingle(const waveForm_t *wf) {
    ...
    if(wf->func == GF_NOISE) {
        glow = wf->base + R_NoiseGet4f(0, 0, 0,
                                       (tess.shaderTime + wf->phase) * wf->frequency) * wf->amplitude;
    } else {
    ...
    for(i = 0, v = tess.xyz[0] ; i < tess.numVertexes ; i++, v += 4) {
        ...
        if(eyeOutside) {
            if(t < 1.0) {
                ...
                t = 1.0 / 32 + 30.0 / 32 * t / (t -
                                                eyeT);   // cut the distance at the fog plane
            }
        } else {
            if(t < 0) {
                t = 1.0 / 32;   // point is outside, so no fogging
    ...
    if(shader->numDeforms) {
        ...
        switch(ds->deformation) {
            ...
            case DEFORM_BULGE:
                ...
                return (backEnd.refdef.floatTime != static_cast<float32>
