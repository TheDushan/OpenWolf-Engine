...
static bool R_LoadMD3( model_t* mod, S32 lod, void* buffer, S32 bufferSize, StringEntry modName );
static bool R_LoadMDR( model_t* mod, void* buffer, S32 filesize, StringEntry name );
...
====================
*/
qhandle_t R_RegisterMD3( StringEntry name, model_t* mod )
{
    union
    {
        U32* u;
    ...
    bool	loaded = false;
    S32			numLoaded;
    UTF8 filename[MAX_QPATH], namebuf[MAX_QPATH + 20];
    UTF8* fext, defex[] = "md3";
...
====================
*/
qhandle_t R_RegisterMDR( StringEntry name, model_t* mod )
{
    union
    {
        U32* u;
...
====================
*/
qhandle_t R_RegisterIQM( StringEntry name, model_t* mod )
{
    union
    {
        U32* u;
...
static bool R_LoadMD3( model_t* mod, S32 lod, void* buffer, S32 bufferSize, StringEntry modName )
{
    ...
    mod->dataSize += size;
    mdvModel = mod->mdv[lod] = reinterpret_cast<mdvModel_t*>( Hunk_Alloc( sizeof( mdvModel_t ), h_low ) );
...
//  ::memcpy(mod->md3[lod], buffer, LittleLong(md3Model->ofsEnd));
    ...
    for( i = 0; i < md3Model->numSurfaces; i++ )
    {
        ...
        surf->shaderIndexes = shaderIndex = reinterpret_cast<S32*>( Hunk_Alloc( sizeof( *shaderIndex ) * md3Surf->numShaders, h_low ) );
        ...
        surf->indexes = tri = reinterpret_cast<U32*>( Hunk_Alloc( sizeof( *tri ) * 3 * md3Surf->numTriangles, h_low ) );
        ...
        // calc tangent spaces
        {
            vec3_t* sdirs = ( vec3_t* )CL_RefMalloc( sizeof( *sdirs ) * surf->numVerts * mdvModel->numFrames );
            vec3_t* tdirs = ( vec3_t* )CL_RefMalloc( sizeof( *tdirs ) * surf->numVerts * mdvModel->numFrames );
    ...
    
    {
        ...
        mdvModel->vaoSurfaces = reinterpret_cast<srfVaoMdvMesh_t*>( Hunk_Alloc( sizeof( *mdvModel->vaoSurfaces ) * mdvModel->numSurfaces, h_low ) );
        ...
        for( i = 0; i < mdvModel->numSurfaces; i++, vaoSurf++, surf++ )
        {
            ...
            vaoSurf->vao = R_CreateVao( va( "staticMD3Mesh_VAO '%s'", surf->name ), data, dataSize, ( U8* )surf->indexes, surf->numIndexes * sizeof( *surf->indexes ), VAO_USAGE_STATIC );
...
=================
*/
static bool R_LoadMDR( model_t* mod, void* buffer, S32 filesize, StringEntry mod_name )
{
    S32					i, j, k, l;
    ...
    {
        CL_RefPrintf( PRINT_WARNING, "R_LoadMDR: %s has wrong version (%i should be %i)\n", mod_name, pinmodel->version, MDR_VERSION );
        return false;
    }
    ...
    if( size > filesize )
    {
        CL_RefPrintf( PRINT_WARNING, "R_LoadMDR: Header of %s is broken. Wrong filesize declared!\n", mod_name );
        return false;
    }
    ...
    mod->dataSize += size;
    mod->modelData = mdr = reinterpret_cast<mdrHeader_t*>( Hunk_Alloc( size, h_low ) );
    ...
    for( l = 0 ; l < mdr->numLODs ; l++ )
    {
        ...
        for( i = 0 ; i < lod->numSurfaces ; i++ )
        {
            ...
            {
                CL_RefPrintf( PRINT_WARNING, "R_LoadMDR: %s has more than %i verts on %s (%i).\n",
            ...
            {
                CL_RefPrintf( PRINT_WARNING, "R_LoadMDR: %s has more than %i triangles on %s (%i).\n",
...
================
*/
static mdvTag_t* R_GetTag( mdvModel_t* mod, S32 frame, StringEntry _tagName )
{
    S32             i;
    mdvTag_t*       tag;
