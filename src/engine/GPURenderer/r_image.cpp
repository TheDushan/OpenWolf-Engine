...
image_t* R_CreateImage2( pointer name, uchar8* pic, sint width, sint height, uint picFormat, sint numMips, imgType_t type, sint/*imgFlags_t*/ flags, sint internalFormat )
{
    ...
    image = tr.images[tr.numImages] = reinterpret_cast<image_t*>( Hunk_Alloc( sizeof( image_t ), h_low ) );
...
qhandle_t idRenderSystemLocal::RegisterSkin( pointer name )
{
    ...
    if( !name || !name[0] )
    {
        CL_RefPrintf( PRINT_DEVELOPER, "Empty name passed to idRenderSystemLocal::RegisterSkin\n" );
        return 0;
    }
    ...
    {
        CL_RefPrintf( PRINT_WARNING, "WARNING: idRenderSystemLocal::RegisterSkin( '%s' ) MAX_SKINS hit\n", name );
        return 0;
    }
    tr.numSkins++;
    ...
    {
        skin->numSurfaces = 1;
        skin->surfaces = reinterpret_cast<skinSurface_t*>( Hunk_Alloc( sizeof( skinSurface_t ), h_low ) );
    ...
    {
        CL_RefPrintf( PRINT_WARNING, "WARNING: Ignoring excess surfaces (found %d, max is %d) in skin '%s'!\n",
    ...
    skin->surfaces = reinterpret_cast<skinSurface_t*>( Hunk_Alloc( skin->numSurfaces * sizeof( skinSurface_t ), h_low ) );
