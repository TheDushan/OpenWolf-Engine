...
static bool R_CullSurface(msurface_t *surf) {
    ...
    if(surf->cullinfo.type & CULLINFO_PLANE) {
        ...
        */
        ...
        if(tr.viewParms.flags & VPF_ORTHOGRAPHIC) {
            d = DotProduct(tr.viewParms.orientation.axis[0],
                           surf->cullinfo.plane.normal);
    ...
    if(surf->cullinfo.type & CULLINFO_SPHERE) {
        ...
        if(tr.currentEntityNum != REFENTITYNUM_WORLD) {
            sphereCull = R_CullLocalPointAndRadius(surf->cullinfo.localOrigin,
                                                   surf->cullinfo.radius);
        } else {
            sphereCull = R_CullPointAndRadius(surf->cullinfo.localOrigin,
                                              surf->cullinfo.radius);
        }
...
static sint R_DlightSurface(msurface_t *surf, sint dlightBits) {
    ...
    if(surf->cullinfo.type & CULLINFO_SPHERE) {
        for(i = 0 ; i < tr.refdef.num_dlights ; i++) {
            ...
            if(!SpheresIntersect(dl->origin, dl->radius, surf->cullinfo.localOrigin,
                                 surf->cullinfo.radius)) {
                // dlight doesn't reach the bounds
                dlightBits &= ~(1 << i);
            }
...
static sint R_PshadowSurface(msurface_t *surf, sint pshadowBits) {
    ...
    if(surf->cullinfo.type & CULLINFO_BOX) {
        for(i = 0 ; i < tr.refdef.num_pshadows ; i++) {
            ...
            if(ps->lightOrigin[0] - ps->lightRadius > surf->cullinfo.bounds[1][0]
                    ...
                    || collisionModelManager->BoxOnPlaneSide(surf->cullinfo.bounds[0],
                            surf->cullinfo.bounds[1], &ps->cullPlane) == 2) {
    ...
    if(surf->cullinfo.type & CULLINFO_SPHERE) {
        for(i = 0 ; i < tr.refdef.num_pshadows ; i++) {
            ...
            if(!SpheresIntersect(ps->viewOrigin, ps->viewRadius,
...
======================
*/
static void R_AddWorldSurface(msurface_t *surf, sint dlightBits,
                              sint pshadowBits) {
    // FIXME: bmodel fog?
...
================
*/
static void R_RecursiveWorldNode(mnode_t *node, uint planeBits,
                                 uint dlightBits, uint pshadowBits) {
    ...
    do {
        ...
        if(!r_nocull->integer) {
            ...
            if(planeBits & 1) {
                r = collisionModelManager->BoxOnPlaneSide(node->mins, node->maxs,
        ...
        if(pshadowBits) {
            ...
            for(i = 0 ; i < tr.refdef.num_pshadows ; i++) {
                ...
                if(pshadowBits & (1 << i)) {
                    ...
                    dist = DotProduct(shadow->lightOrigin,
...

    {
        ...
        tr.viewParms.visBounds[0][0] = MIN(node->mins[0],
                                           tr.viewParms.visBounds[0][0]);
        tr.viewParms.visBounds[0][1] = MIN(node->mins[1],
                                           tr.viewParms.visBounds[0][1]);
        tr.viewParms.visBounds[0][2] = MIN(node->mins[2],
                                           tr.viewParms.visBounds[0][2]);

        tr.viewParms.visBounds[1][0] = MAX(node->maxs[0],
                                           tr.viewParms.visBounds[1][0]);
        tr.viewParms.visBounds[1][1] = MAX(node->maxs[1],
                                           tr.viewParms.visBounds[1][1]);
        tr.viewParms.visBounds[1][2] = MAX(node->maxs[2],
...
bool idRenderSystemLocal::inPVS(const vec3_t p1, const vec3_t p2) {
    ...
    vis = collisionModelManager->ClusterPVS(
              leaf->cluster);   // why not R_ClusterPVS ??
