...
=============
*/
void idNetworkSystemLocal::NetadrToSockadr(netadr_t *a,
        struct sockaddr *s) {
    if(a->type == NA_BROADCAST) {
        ...
        ((struct sockaddr_in *)s)->sin_family = AF_INET;
        ((struct sockaddr_in *)s)->sin_addr.s_addr = *reinterpret_cast<sint *>
                (&a->ip);
        ((struct sockaddr_in *)s)->sin_port = a->port;
...
=============
*/
void idNetworkSystemLocal::SockadrToNetadr(struct sockaddr *s,
        netadr_t *a) {
    if(s->sa_family == AF_INET) {
        a->type = NA_IP;
        *reinterpret_cast<sint *>(&a->ip) = ((struct sockaddr_in *)
                                             s)->sin_addr.s_addr;
...
=============
*/
struct addrinfo *idNetworkSystemLocal::SearchAddrInfo(
    struct addrinfo *hints, sa_family_t family) {
    while(hints) {
...
=============
*/
bool idNetworkSystemLocal::StringToSockaddr(pointer s,
        struct sockaddr *sadr, sint sadr_len, sa_family_t family) {
    ...
    if(!retval) {
        ...
        if(search) {
            ...
            common->Printf("idNetworkSystemLocal::StringToSockaddr: Error resolving %s: No address of required type found.\n",
                           s);
        }
    } else {
        common->Printf("idNetworkSystemLocal::StringToSockaddr: Error resolving %s: %s\n",
...
=============
*/
void idNetworkSystemLocal::SockaddrToString(valueType *dest,
        uint64 destlen, struct sockaddr *input) {
    socklen_t inputlen;
    ...
    if(getnameinfo(input, inputlen, dest, static_cast<uint>(destlen), nullptr,
                   0, NI_NUMERICHOST) && destlen > 0) {
        *dest = '\0';
    }
}
...
=============
*/
bool idNetworkSystemLocal::StringToAdr(pointer s, netadr_t *a,
                                       netadrtype_t family) {
...
===================
*/
bool idNetworkSystemLocal::CompareBaseAdrMask(netadr_t a, netadr_t b,
        sint netmask) {
    bool differed;
    ...
    if(a.type == NA_IP) {
        ...
        common->Printf("idNetworkSystemLocal::CompareBaseAdr: bad address type\n");
        return false;
    }
...
pointer idNetworkSystemLocal::AdrToString(netadr_t a) {
    ...
    switch(a.type) {
        ...
        case NA_IP:
            Q_vsprintf_s(s, sizeof(s), sizeof(s), "%i.%i.%i.%i", a.ip[0], a.ip[1],
                         a.ip[2], a.ip[3]);
            break;
        ...
        default:
            common->Printf("idNetworkSystemLocal::AdrToString: Unknown address type: %i\n",
                           a.type);
...
pointer idNetworkSystemLocal::AdrToStringwPort(netadr_t a) {
    ...
    switch(a.type) {
        ...
        case NA_IP:
            Q_vsprintf_s(s, sizeof(s), sizeof(s), "%i.%i.%i.%i:%hu", a.ip[0], a.ip[1],
                         a.ip[2], a.ip[3], BigShort(a.port));
            break;

        case NA_IP6:
            Q_vsprintf_s(s, sizeof(s), sizeof(s), "[%s]:%hu", AdrToString(a),
                         ntohs(a.port));
            break;
        ...
        default:
            common->Printf("idNetworkSystemLocal::AdrToStringwPort: Unknown address type: %i\n",
                           a.type);
            Q_vsprintf_s(s, sizeof(s), sizeof(s), "unknown");
            break;
    }
...
bool idNetworkSystemLocal::GetPacket(netadr_t *net_from,
                                     msg_t *net_message) {
    sint    ret;
    ...
    if(ip_socket != INVALID_SOCKET) {
        fromlen = sizeof(from);
        ret = recvfrom(ip_socket, reinterpret_cast<valueType *>(net_message->data),
                       net_message->maxsize, 0, (struct sockaddr *) &from, &fromlen);

        if(ret == SOCKET_ERROR) {
            err = socketError;

            if(err != EAGAIN && err != ECONNRESET) {
                common->Printf("idNetworkSystemLocal::GetPacket: %s\n", ErrorString());
            }
        } else {
    ...
    if(ip6_socket != INVALID_SOCKET) {
        ...
        ret = recvfrom(ip6_socket,
                       reinterpret_cast<valueType *>(net_message->data), net_message->maxsize, 0,
...
==================
*/
void idNetworkSystemLocal::SendPacket(sint length, const void *data,
                                      netadr_t to) {
    ...
    if(to.type != NA_BROADCAST && to.type != NA_IP && to.type != NA_IP6 &&
        ...
        common->Error(ERR_FATAL,
                      "idNetworkSystemLocal::SendPacket: bad address type");
        return;
    }
    ...
    if(usingSocks && to.type == NA_IP) {
        ...
        *reinterpret_cast<sint *>(&socksBuf[4]) = ((struct sockaddr_in *)
                &addr)->sin_addr.s_addr;
        *reinterpret_cast<schar16 *>(&socksBuf[8]) = ((struct sockaddr_in *)
                &addr)->sin_port;
        ...
        if(addr.ss_family == AF_INET) {
            ret = sendto(ip_socket, reinterpret_cast<pointer>(data), length, 0,
                         (struct sockaddr *) &addr, sizeof(struct sockaddr_in));
        } else if(addr.ss_family == AF_INET6) {
            ret = sendto(ip6_socket, reinterpret_cast<pointer>(data), length, 0,
    ...
    if(ret == SOCKET_ERROR) {
        ...
        common->Printf("idNetworkSystemLocal::SendPacket: %s\n", ErrorString());
    }
}
...
bool idNetworkSystemLocal::IsLANAddress(netadr_t adr) {
    ...
    for(index = 0; index < numIP; index++) {
        if(localIP[index].type == adr.type) {
            if(adr.type == NA_IP) {
                compareip = reinterpret_cast<uchar8 *>(& ((struct sockaddr_in *)
                                                       &localIP[index].addr)->sin_addr.s_addr);
                comparemask = reinterpret_cast<uchar8 *>(& ((struct sockaddr_in *)
                ...
                compareip = reinterpret_cast<uchar8 *>(& ((struct sockaddr_in6 *)
                                                       &localIP[index].addr)->sin6_addr);
                comparemask = reinterpret_cast<uchar8 *>(& ((struct sockaddr_in6 *)
            ...
            for(run = 0; run < addrsize; run++) {
                if((compareip[run] & comparemask[run]) != (compareadr[run] &
                        comparemask[run])) {
                    differed = true;
                    break;
                }
...
void idNetworkSystemLocal::ShowIP(void) {
    ...
    for(i = 0; i < numIP; i++) {
        SockaddrToString(addrbuf, sizeof(addrbuf),
...
====================
*/
SOCKET idNetworkSystemLocal::IPSocket(valueType *net_interface, sint port,
                                      sint *err) {
    SOCKET              newsocket;
    ...
    *err = 0;
    ...
    if(net_interface) {
        ...
        common->Printf("Opening IP socket: 0.0.0.0:%i\n", port);
    }

    if((newsocket = socket(PF_INET, SOCK_DGRAM,
                           IPPROTO_UDP)) == INVALID_SOCKET) {
        *err = socketError;
        common->Printf("WARNING: idNetworkSystemLocal::IPSocket: socket: %s\n",
                       ErrorString());
        return newsocket;
    }
    ...
    if(ioctlsocket(newsocket, FIONBIO, &_true) == SOCKET_ERROR) {
        common->Printf("WARNING: idNetworkSystemLocal::IPSocket: ioctl FIONBIO: %s\n",
                       ErrorString());
        *err = socketError;
    ...
    if(setsockopt(newsocket, SOL_SOCKET, SO_BROADCAST,
        ...
        common->Printf("WARNING: idNetworkSystemLocal::IPSocket: setsockopt SO_BROADCAST: %s\n",
                       ErrorString());
    }
    ...
    if(!net_interface || !net_interface[0]) {
        ...
        if(!StringToSockaddr(net_interface, (struct sockaddr *)&address,
    ...
    if(bind(newsocket, (struct sockaddr *)&address,
            sizeof(address)) == SOCKET_ERROR) {
        common->Printf("WARNING: idNetworkSystemLocal::IPSocket: bind: %s\n",
                       ErrorString());
        *err = socketError;
        closesocket(newsocket);
        return INVALID_SOCKET;
...
====================
*/
SOCKET idNetworkSystemLocal::IP6Socket(valueType *net_interface, sint port,
    ...
    *err = 0;
    ...
    if(net_interface) {
        ...
        if(Q_CountChar(net_interface, ':')) {
            common->Printf("Opening IP6 socket: [%s]:%i\n", net_interface, port);
        } else {
            common->Printf("Opening IP6 socket: %s:%i\n", net_interface, port);
        }
    } else {
        common->Printf("Opening IP6 socket: [::]:%i\n", port);
    }

    if((newsocket = socket(PF_INET6, SOCK_DGRAM,
                           IPPROTO_UDP)) == INVALID_SOCKET) {
        *err = socketError;
        common->Printf("WARNING: idNetworkSystemLocal::IP6Socket: socket: %s\n",
                       ErrorString());
        return newsocket;
    }
    ...
    // make it non-blocking
    if(ioctlsocket(newsocket, FIONBIO, &_true) == SOCKET_ERROR) {
        common->Printf("WARNING: idNetworkSystemLocal::IP6Socket: ioctl FIONBIO: %s\n",
                       ErrorString());
        *err = socketError;
        closesocket(newsocket);
        return INVALID_SOCKET;
...
#ifdef IPV6_V6ONLY
    {
        ...
        if(setsockopt(newsocket, IPPROTO_IPV6, IPV6_V6ONLY,
            ...
            if(developer->integer) {
                common->Printf("WARNING: idNetworkSystemLocal::IP6Socket: setsockopt IPV6_V6ONLY: %s\n",
                               ErrorString());
            }
        }
    }
    ...
    if(bind(newsocket, (struct sockaddr *)&address,
            sizeof(address)) == SOCKET_ERROR) {
        common->Printf("WARNING: idNetworkSystemLocal::IP6Socket: bind: %s\n",
                       ErrorString());
        *err = socketError;
        closesocket(newsocket);
        return INVALID_SOCKET;
...
void idNetworkSystemLocal::SetMulticast6(void) {
    ...
    if(!*net_mcast6addr->string ||
            !StringToSockaddr(net_mcast6addr->string, (struct sockaddr *) &addr,
                              sizeof(addr), AF_INET6)) {
        common->Printf("WARNING: idNetworkSystemLocal::JoinMulticast6: Incorrect multicast address given, "
                       "please set cvar %s to a sane value.\n", net_mcast6addr->name);

        cvarSystem->SetValue(net_enabled->name,
...
void idNetworkSystemLocal::JoinMulticast6(void) {
    ...
    if(IN6_IS_ADDR_MULTICAST(&boundto.sin6_addr) ||
        ...
        if((multicast6_socket = IP6Socket(net_mcast6addr->string,
                                          ntohs(boundto.sin6_port), nullptr, &err)) == INVALID_SOCKET) {
    ...
    if(curgroup.ipv6mr_interface) {
        if(setsockopt(multicast6_socket, IPPROTO_IPV6, IPV6_MULTICAST_IF,
                      reinterpret_cast<valueType *>(&curgroup.ipv6mr_interface),
                      sizeof(curgroup.ipv6mr_interface)) < 0) {
            common->Printf("idNetworkSystemLocal::JoinMulticast6: Couldn't set scope on multicast socket: %s\n",
                           ErrorString());
    ...
    if(setsockopt(multicast6_socket, IPPROTO_IPV6, IPV6_JOIN_GROUP,
                  reinterpret_cast<valueType *>(&curgroup), sizeof(curgroup))) {
        common->Printf("idNetworkSystemLocal::JoinMulticast6: Couldn't join multicast group: %s\n",
                       ErrorString());
...
void idNetworkSystemLocal::OpenSocks(sint port) {
    ...
    if((socks_socket = socket(AF_INET, SOCK_STREAM,
                              IPPROTO_TCP)) == INVALID_SOCKET) {
        err = socketError;
        common->Printf("WARNING: idNetworkSystemLocal::OpenSocks: socket: %s\n",
                       ErrorString());
        return;
    }
    ...
    if(h == nullptr) {
        err = socketError;
        common->Printf("WARNING: idNetworkSystemLocal::OpenSocks: gethostbyname: %s\n",
                       ErrorString());
        return;
    }

    if(h->h_addrtype != AF_INET) {
        common->Printf("WARNING: idNetworkSystemLocal::OpenSocks: gethostbyname: address type was not AF_INET\n");
        return;
    }
    ...
    if(connect(socks_socket, (struct sockaddr *)&address,
               sizeof(address)) == SOCKET_ERROR) {
        err = socketError;
        common->Printf("idNetworkSystemLocal::OpenSocks: connect: %s\n",
                       ErrorString());
        return;
    }
    ...
    if(send(socks_socket, reinterpret_cast<valueType *>(buf), len,
            0) == SOCKET_ERROR) {
        err = socketError;
        common->Printf("idNetworkSystemLocal::OpenSocks: send: %s\n",
                       ErrorString());
        return;
    }
    ...
    if(len == SOCKET_ERROR) {
        err = socketError;
        common->Printf("idNetworkSystemLocal::OpenSocks: recv: %s\n",
                       ErrorString());
        return;
    }

    if(len != 2 || buf[0] != 5) {
        common->Printf("idNetworkSystemLocal::OpenSocks: bad response\n");
        return;
    }
    ...
    switch(buf[1]) {
        ...
        default:
            common->Printf("idNetworkSystemLocal::OpenSocks: request denied\n");
            return;
    }
    ...
    if(buf[1] == 2) {
        ...
        // send it
        if(send(socks_socket, reinterpret_cast<pointer>(buf),
                static_cast<sint>(3 + ulen + plen), 0) == SOCKET_ERROR) {
            err = socketError;
            common->Printf("idNetworkSystemLocal::OpenSocks: send: %s\n",
                           ErrorString());
            return;
        ...
        if(buf[1] != 0) {
            common->Printf("idNetworkSystemLocal::OpenSocks: authentication failed\n");
            return;
        }
    }
    ...
    *reinterpret_cast<schar16 *>(&buf[8]) = htons((schar16)
                                            port);          // port

    if(send(socks_socket, reinterpret_cast<valueType *>(buf), 10,
            0) == SOCKET_ERROR) {
        err = socketError;
        common->Printf("idNetworkSystemLocal::OpenSocks: send: %s\n",
                       ErrorString());
    ...
    if(buf[1] != 0) {
        common->Printf("idNetworkSystemLocal::OpenSocks: request denied: %i\n",
                       buf[1]);
        return;
    }

    if(buf[3] != 1) {
        common->Printf("idNetworkSystemLocal::OpenSocks: relay address is not IPV4: %i\n",
                       buf[3]);
        return;
    }
    ...
    ((struct sockaddr_in *)&socksRelayAddr)->sin_addr.s_addr =
        *reinterpret_cast<sint *>(&buf[4]);
    ((struct sockaddr_in *)&socksRelayAddr)->sin_port =
        *reinterpret_cast<schar16 *>(&buf[8]);
...
=====================
*/
void idNetworkSystemLocal::AddLocalAddress(valueType *ifname,
...
void idNetworkSystemLocal::GetLocalAddress(void) {
    ...
    if(getifaddrs(&ifap)) {
        common->Printf("idNetworkSystemLocal::GetLocalAddress: Unable to get list of network interfaces: %s\n",
                       ErrorString());
    } else {
...
void idNetworkSystemLocal::OpenIP(void) {
    ...
    if(net_enabled->integer & NET_ENABLEV6) {
        ...
        if(ip6_socket == INVALID_SOCKET) {
            common->Printf("WARNING: Couldn't bind to a v6 ip address.\n");
        }
    }
    ...
    if(net_enabled->integer & NET_ENABLEV4) {
        ...
        if(ip_socket == INVALID_SOCKET) {
            common->Printf("WARNING: Couldn't bind to a v4 ip address.\n");
        }
    }
}
...
void idNetworkSystemLocal::Init(void) {
    ...
    cmdSystem->AddCommand("net_restart", NETRestart_f,
                          "If you change any net_ setting in-game or in a config you need to also to a net_restart to make the changes take effect, i have seen very few people with this in their configurations, but i have seen some people specify a lot of net_ settings and they have it in their config.");
}
...
sint idNetworkSystemLocal::ConnectTCP(valueType *s_host_port) {
    ...
    if(s_server == nullptr) {
        common->Printf("Error parsing server string %s does not have port\n",
                       s_host_port);
        return -1;
    }
    ...
    if(s_port == nullptr) {
        ...
        common->Printf("Error parsing server string %s port problem\n",
                       s_host_port);
    }

    if((sock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) == INVALID_SOCKET) {
        err = socketError;
        common->Printf("WARNING: idNetworkSystemLocal::ConnectTCP: socket: %s\n",
                       ErrorString());
        return -1;
    }
    ...
    if(h == nullptr) {
        err = socketError;
        common->Printf("WARNING: idNetworkSystemLocal::ConnectTCP: gethostbyname: %s\n",
                       ErrorString());
        close(sock);
        return -1;
    }
    ...
    if(h->h_addrtype != AF_INET) {
        common->Printf("WARNING: idNetworkSystemLocal::ConnectTCP: gethostbyname: address type was not AF_INET\n");
        close(sock);
        return -1;
    }
    ...
    if(connect(sock, (struct sockaddr *)&address,
        ...
        err = socketError;
#ifdef _DEBUG
        common->Printf("idNetworkSystemLocal::ConnectTCP: connect: %s\n",
                       ErrorString());
#endif
        return -1;
    }
