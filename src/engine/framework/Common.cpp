...
=============
*/
void idCommonLocal::BeginRedirect(valueType *buffer, uint64 buffersize,
...
===============
*/
void idCommonLocal::EndRedirect_f(void) {
    static_cast<idCommonLocal *>(common)->EndRedirect();
}
...
=============
*/
void idCommonLocal::Printf(pointer fmt, ...) {
    va_list argptr;
    ...
    if(logfile && logfile->integer) {
        ...
        if(!com_logfile && fileSystem->Initialized() && !opening_qconsole) {
            ...
            if(logfile) {
                ...
                Printf("Opening qconsole.log failed!\n");
...
=============
*/
void idCommonLocal::Error(errorParm_t code, pointer fmt, ...) {
    va_list argptr;
    ...
    switch(code) {
        ...
        case ERR_AUTOUPDATE: {
            ...
            if(!Q_stricmpn(com_errorMessage, "Server is full", 14) &&
                    clientAutoUpdateSystem->NextUpdateServer()) {
    ...
    Shutdown(code == ERR_VID_FATAL ? true : false);
...
/*
==============
idCommonLocal::Colors_f
==============
*/
void idCommonLocal::Colors_f(void) {
    sint row, i, j;

    commonLocal.Printf("^3 %-20s %-6s %-20s %-6s^7\n\n", "Color", "Code",
                       "Color", "Code");

    for(row = 0;
            row < ((sint)ceilf(((float32)color_definitions_length) / 2.0f)); row++) {
        for(i = (row * 2), j = 0; i < color_definitions_length &&
                j < 2; i++, j++) {
            commonLocal.Printf(
                " ^%s%-20s ^^%-5s",
    ...
    commonLocal.Printf("^3The format for the hardcoded standard color codes is ^5^^x^3, where x is alphanumeric (^7[^50-9^7][^5a-z^7][^5A-Z^7]^3).\n\n");
    commonLocal.Printf("^3The short format for custom hexadecimal defined color codes is ^5^^#xxx^3, where x is a hexadecimal (^7[^50-9^7][^5a-f^7][^5A-F^7]^3).\n\n");
    commonLocal.Printf("^3The long format for custom hexadecimal defined color codes (which has even more possible colors) is ^5^^##xxxxxx^3, where x is a hexadecimal (^7[^50-9^7][^5a-f^7][^5A-F^7]^3).^7\n\n\n");
}

/*
=============
idCommonLocal::Quit_f
...
==================
*/
void idCommonLocal::ParseCommandLine(valueType *commandLine) {
...
bool idCommonLocal::SafeMode(void) {
    ...
    for(i = 0; i < com_numConsoleLines; i++) {
        ...
        if(!Q_stricmp(cmdSystem->Argv(0), "safe") ||
                !Q_stricmp(cmdSystem->Argv(0), "cvar_restart")) {
...
===============
*/
void idCommonLocal::StartupVariable(pointer match) {
    sint             i;
    valueType           *s;
...
void idCommonLocal::InfoPrint(pointer s) {
...
/*
============
idCommonLocal::Filter
============
*/
sint idCommonLocal::Filter(valueType *filter, valueType *name,
                           sint casesensitive) {
...
/*
============
idCommonLocal::FilterPath
============
*/
sint idCommonLocal::FilterPath(pointer filter, pointer name,
                               sint casesensitive) {
    sint             i;
...
/*
================
idCommonLocal::RealTime
================
*/
sint idCommonLocal::RealTime(qtime_t *qtime) {
    time_t          t;
    struct tm      *tms;
...
/*
=================
idCommonLocal::InitJournaling
=================
*/
void idCommonLocal::InitJournaling(void) {
    sint i;

    commonLocal.StartupVariable("journal");
    ...
    if(!com_journalFile || !com_journalDataFile) {
        ...
        commonLocal.Printf("Couldn't open journal files\n");
    }
}
...
pointer idCommonLocal::ShowEventName(sysEventType_t eventType) {
...
================
*/
void idCommonLocal::QueueEvent(sint evTime, sysEventType_t evType,
                               sint value, sint value2, sint ptrLength, void *ptr) {
    sysEvent_t  *ev;
    ...
    if(eventHead - eventTail >= MAX_QUEUED_EVENTS) {
        Printf("idCommonLocal::QueueEvent(%s, time=%i): overflow\n",
               ShowEventName(evType), evTime);
...
/*
================
idCommonLocal::GetSystemEvent
================
*/
sysEvent_t idCommonLocal::GetSystemEvent(void) {
    sysEvent_t  ev;
    valueType        *s;
    msg_t       netmsg;
    netadr_t    adr;
...
/*
=================
idCommonLocal::GetRealEvent
=================
*/
sysEvent_t idCommonLocal::GetRealEvent(void) {
    sint r;
    sysEvent_t ev;
    ...
    if(journal->integer == 2) {
        ...
        if(r != sizeof(ev)) {
            //Error( ERR_FATAL, "Error reading from journal file" );
            journal->integer = 0;
            ev.evType = SYSE_NONE;
        }
        ...
        if(journal->integer == 1) {
            ...
            if(r != sizeof(ev)) {
                commonLocal.Error(ERR_FATAL, "Error writing to journal file");
            }
...
/*
=================
idCommonLocal::PushEvent
=================
*/
void idCommonLocal::PushEvent(sysEvent_t *_event) {
    sysEvent_t     *ev;
    static sint      printedWarning =
        0;    // bk001129 - init, bk001204 - explicit sint
    ...
    if(com_pushedEventsHead - com_pushedEventsTail >= MAX_PUSHED_EVENTS) {
        ...
        if(!printedWarning) {
            printedWarning = true;
            commonLocal.Printf("WARNING: idCommonLocal::PushEvent overflow\n");
        }
...
/*
=================
idCommonLocal::GetEvent
=================
*/
sysEvent_t idCommonLocal::GetEvent(void) {
...
/*
=================
idCommonLocal::RunAndTimeServerPacket
=================
*/
void idCommonLocal::RunAndTimeServerPacket(netadr_t *evFrom, msg_t *buf) {
    sint             t1, t2, msec;
    ...
    if(com_speeds->integer) {
        ...
        if(com_speeds->integer == 3) {
            commonLocal.Printf("idServerMainSystemLocal::PacketEvent time: %i\n",
                               msec);
        }
    }
}
...
/*
=================
idCommonLocal::EventLoop

Returns last event time
=================
*/

sint idCommonLocal::EventLoop(void) {
    sysEvent_t      ev;
    netadr_t        evFrom;
    ...
    while(1) {
        ...
        if(ev.evType == SYSE_NONE) {
            ...
            while(networkChainSystem->GetLoopPacket(NS_SERVER, &evFrom, &buf)) {
                ...
                if(sv_running->integer) {
                    RunAndTimeServerPacket(&evFrom, &buf);
                }
            }
        ...
        switch(ev.evType) {
            default:
                ...
                Error(ERR_FATAL, "idCommonLocal::EventLoop: bad event type %i", ev.evType);
                break;
            ...
            case SYSE_CONSOLE:
                ...
                //cmdBufferSystem->AddText( "\n" );
                if((static_cast<valueType *>(ev.evPtr))[0] == '\\' ||
                        (static_cast<valueType *>(ev.evPtr))[0] == '/') {
            ...
            case SYSE_PACKET:
                ...
                if(static_cast<uint>(buf.cursize) > buf.maxsize) {
                    Printf("idCommonLocal::EventLoop: oversize packet\n");
                    continue;
                }

                memcpy(buf.data, reinterpret_cast<uchar8 *>((netadr_t *)ev.evPtr + 1),
                       buf.cursize);
...
/*
================
idCommonLocal::Milliseconds

Can be used for profiling, but will be journaled accurately
================
*/
sint idCommonLocal::Milliseconds(void) {
    sysEvent_t ev;
...
/*
=============
idCommonLocal::Error_f

Just throw a fatal error to
test error shutdown procedures
=============
*/
void idCommonLocal::Error_f(void) {
    if(cmdSystem->Argc() > 1) {
        commonLocal.Error(ERR_DROP, "Testing drop error");
    } else {
        commonLocal.Error(ERR_FATAL, "Testing fatal error");
    }
}

/*
=============
idCommonLocal::Freeze_f

Just freeze in place for a given number of seconds to test
error recovery
=============
*/
void idCommonLocal::Freeze_f(void) {
    float32           s;
    sint             start, now;
    ...
    start = commonLocal.Milliseconds();

    while(1) {
        now = commonLocal.Milliseconds();
...
/*
=================
idCommonLocal::Crash_f

A way to force a bus error for development reasons
=================
*/
void idCommonLocal::Crash_f(void) {
    *(volatile sint *)0 = 0x12345678;
}

/*
=================
idCommonLocal::SetRecommended
=================
*/
void idCommonLocal::SetRecommended(void) {
    bool goodVideo;
    ...
    if(goodVideo) {
        Printf("Found high quality video and high quality CPU\n");
        cmdBufferSystem->AddText("exec ultra.cfg\n");
        cvarSystem->Set("com_recommended", "2");
    } else {
        Printf("Found low quality video and low quality CPU\n");
...
=================
*/
void idCommonLocal::GetGameInfo(void) {
    valueType           *f, *buf;
    valueType           *token;
        ...
        while((token = COM_Parse(&buf)) != nullptr && token[0]) {
            if(!Q_stricmp(token, "spEnabled")) {
                ...
                if((token = COM_ParseExt(&buf, false)) != nullptr && token[0]) {
                    ...
                    commonLocal.Error(ERR_FATAL, "idCommonLocal::GetGameInfo: bad syntax.");
                }
...
=================
*/
bool idCommonLocal::CheckProfile(valueType *profile_path) {
    fileHandle_t    f;
    valueType            f_data[32];
    sint             f_pid;
...
=================
*/
void idCommonLocal::TrackProfile(valueType *profile_path) {
    valueType temp_fs_gamedir[MAX_OSPATH];

    //  Printf( "idCommonLocal::TrackProfile: Tracking profile [%s] [%s]\n", fs_gamedir, profile_path );
    ...
    if(strcmp(last_fs_gamedir, fs_gamedir)) {
        if(strlen(last_fs_gamedir) && strlen(last_profile_path)) {
            ...
            if(fileSystem->FileExists(last_profile_path)) {
                commonLocal.Printf("idCommonLocal::TrackProfile: Deleting old pid file [%s] [%s]\n",
...
=================
*/
bool idCommonLocal::WriteProfile(valueType *profile_path) {
    fileHandle_t    f;
    ...
    if(f < 0) {
        Printf("idCommonLocal::WriteProfile: Can't write %s.\n", profile_path);
        return false;
    }
...
=================
*/
void idCommonLocal::Init(valueType *commandLine) {
    valueType           *s;
    sint             pid;
    ...
    Printf("%s %s %s\n%s\n", PRODUCT_NAME, PLATFORM_STRING, __DATE__,
           commandLine);
    ...
    StartupVariable("com_ignorecrash");
    ...
    if(!SafeMode()) {
        ...
        if(com_gameInfo.usesProfiles) {
            if(!cl_profileStr[0]) {
                ...
                fileSystem->ReadFile("profiles/defaultprofile.dat",
                                     (void **)&defaultProfile);
    ...
    if(developer && developer->integer) {
        cmdSystem->AddCommand("error", &idCommonLocal::Error_f,
                              "Just throw a fatal error to test error shutdown procedures");
        cmdSystem->AddCommand("crash", &idCommonLocal::Crash_f,
                              "A way to force a bus error for development reasons");
        cmdSystem->AddCommand("freeze", &idCommonLocal::Freeze_f,
                              "Just freeze in place for a given number of seconds to test error recovery");
    }

    cmdSystem->AddCommand("quit", &idCommonLocal::Quit_f,
                          "Quits the running game, and exits application completely. For server also see killserver");
    cmdSystem->AddCommand("colors", &idCommonLocal::Colors_f,
                          "Displays help for working with chat colors");
    ...
    networkChainSystem->Init(Milliseconds());
    ...
    if(!dedicated->integer && !AddStartupCommands()) {
#if 1
...
void idCommonLocal::WriteConfigToFile(pointer filename) {
    fileHandle_t    f;

    f = fileSystem->FOpenFileWrite(filename);

    if(!f) {
        commonLocal.Printf("Couldn't write %s.\n", filename);
        return;
    }
...
/*
===============
idCommonLocal::WriteConfig_f

Write the config file to a specific name
===============
*/
void idCommonLocal::WriteConfig_f(void) {
...
/*
================
idCommonLocal::ModifyMsec
================
*/
sint idCommonLocal::ModifyMsec(sint msec) {
    sint clampTime;
...
void idCommonLocal::Frame(void) {
    ...
    if(dedicated->integer && !sv_running->integer && com_watchdog->integer) {
        if(watchdogTime == 0) {
            ...
            if(!watchWarn &&
                    idsystem->Milliseconds() - watchdogTime > (com_watchdog->integer - 4) *
                    1000) {
                Printf("WARNING: watchdog will trigger in 4 seconds\n");
                watchWarn = true;
            } else if(idsystem->Milliseconds() - watchdogTime > com_watchdog->integer *
                      1000) {
                Printf("Idle Server with no map - triggering watchdog\n");
                watchdogTime = 0;
                watchWarn = false;
    ...
    if(com_speeds->integer) {
        ...
        Printf("frame:%i all:%3i sv:%3i sev:%3i cev:%3i cl:%3i gm:%3i rf:%3i bk:%3i\n",
               com_frameNumber, all, sv, sev, cev, cl, time_game, time_frontend,
               time_backend);
    }
    ...
    if(com_showtrace->integer) {
        ...
        Printf("%4i traces  (%ib %ip %it) %4i points\n", c_traces, c_brush_traces,
               c_patch_traces, c_trisoup_traces, c_pointcontents);
        c_traces = 0;
        c_brush_traces = 0;
        c_patch_traces = 0;
        c_trisoup_traces = 0;
...
=================
*/
void idCommonLocal::Shutdown(bool badProfile) {
    valueType *cl_profileStr = cl_profile->string;
...
/*
==================
idCommonLocal::RandomBytes

fills string array with len radom bytes, peferably from the OS randomizer
==================
*/
void idCommonLocal::RandomBytes(uchar8 *string, sint len) {
    sint i;
    ...
    Printf("idCommonLocal::RandomBytes: using weak randomization\n");
