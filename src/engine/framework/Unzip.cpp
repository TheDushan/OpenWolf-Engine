...
local int unz64local_getShort(const zlib_filefunc64_32_def
                              *pzlib_filefunc_def,
                              voidpf filestream,
                              uLong *pX) {
    uLong x ;
...
local int unz64local_getLong(const zlib_filefunc64_32_def
                             *pzlib_filefunc_def,
                             voidpf filestream,
                             uLong *pX) {
    uLong x ;
...
local int unz64local_getLong64(const zlib_filefunc64_32_def
                               *pzlib_filefunc_def,
                               voidpf filestream,
                               ZPOS64_T *pX) {
    ZPOS64_T x ;
...
local int strcmpcasenosensitive_internal(pointer fileName1,
        pointer fileName2) {
    for(;;) {
        char c1 = *(fileName1++);
        char c2 = *(fileName2++);
...
/*
  Locate the Central directory of a zipfile (at the end, just before
    the global comment)
*/
local ZPOS64_T unz64local_SearchCentralDir OF((const
        zlib_filefunc64_32_def *pzlib_filefunc_def, voidpf filestream));
local ZPOS64_T unz64local_SearchCentralDir(const zlib_filefunc64_32_def
        *pzlib_filefunc_def, voidpf filestream) {
    unsigned char *buf;
    ZPOS64_T uSizeFile;
    ZPOS64_T uBackRead;
...
local ZPOS64_T unz64local_SearchCentralDir64(const zlib_filefunc64_32_def
        *pzlib_filefunc_def,
        voidpf filestream) {
    unsigned char *buf;
    ZPOS64_T uSizeFile;
    ...
    if(unz64local_getLong64(pzlib_filefunc_def, filestream,
                            &relativeOffset) != UNZ_OK) {
        return 0;
    }
...
local int unz64local_GetCurrentFileInfoInternal(unzFile file,
    ...
    if((err == UNZ_OK) && (szComment != nullptr)) {
        ...
        if((file_info.size_file_comment > 0) && (commentBufferSize > 0))
            if(ZREAD64(s->z_filefunc, s->filestream, szComment,
                       uSizeRead) != uSizeRead) {
                err = UNZ_ERRNO;
            }
...
*/
extern int ZEXPORT unzLocateFile(unzFile file, pointer szFileName,
                                 int iCaseSensitivity) {
    unz64_s *s;
    int err;
...
#    endif
    ...
    pfile_in_zip_read_info = (file_in_zip64_read_info_s *)ALLOC(sizeof(
                                 file_in_zip64_read_info_s));
    ...
    pfile_in_zip_read_info->read_buffer = reinterpret_cast<valueType *>(ALLOC(
            UNZ_BUFSIZE));
...
*/
extern int ZEXPORT unzReadCurrentFile(unzFile file, voidp buf,
                                      unsigned len) {
    int err = UNZ_OK;
    uInt iRead = 0;
    unz64_s *s;
...
#            endif
        ...
        if((pfile_in_zip_read_info->compression_method == 0) ||
            ...
            pfile_in_zip_read_info->total_out_64 = pfile_in_zip_read_info->total_out_64
                                                   + uDoCopy;
            ...
            pfile_in_zip_read_info->bstream.next_in        =
                reinterpret_cast<valueType *>(pfile_in_zip_read_info->stream.next_in);
            ...
            pfile_in_zip_read_info->bstream.next_out       =
                reinterpret_cast<valueType *>(pfile_in_zip_read_info->stream.next_out);
            ...
            pfile_in_zip_read_info->crc32 = crc32(pfile_in_zip_read_info->crc32,
                                                  bufBefore, static_cast<uint>(uOutThis));
            pfile_in_zip_read_info->rest_read_uncompressed -= uOutThis;
            iRead += static_cast<uint>(uTotalOutAfter) - uTotalOutBefore;

            pfile_in_zip_read_info->stream.next_in   = (Bytef *)
                    pfile_in_zip_read_info->bstream.next_in;
            pfile_in_zip_read_info->stream.avail_in  =
                pfile_in_zip_read_info->bstream.avail_in;
            pfile_in_zip_read_info->stream.total_in  =
                pfile_in_zip_read_info->bstream.total_in_lo32;
            pfile_in_zip_read_info->stream.next_out  = (Bytef *)
                    pfile_in_zip_read_info->bstream.next_out;
            pfile_in_zip_read_info->stream.avail_out =
                pfile_in_zip_read_info->bstream.avail_out;
            pfile_in_zip_read_info->stream.total_out =
