...
void CM_BoxLeafnums_r(leafList_t *ll, sint nodenum) {
    ...
    while(1) {
        ...
        plane = node->plane;
        s = collisionModelManagerLocal.BoxOnPlaneSide(ll->bounds[0], ll->bounds[1],
                plane);
...
==================
*/
sint idCollisionModelManagerLocal::BoxLeafnums(const vec3_t mins,
        const vec3_t maxs, sint *list, sint listsize, sint *lastLeaf) {
    leafList_t ll;
...
==================
*/
sint CM_BoxBrushes(const vec3_t mins, const vec3_t maxs, cbrush_t **list,
                   sint listsize) {
    leafList_t      ll;
    ...
    ll.count = 0;
    ll.maxcount = listsize;
    ll.list = reinterpret_cast<sint *>(list);
...
==================
*/
sint idCollisionModelManagerLocal::PointContents(const vec3_t p,
        clipHandle_t model) {
...
==================
*/
sint idCollisionModelManagerLocal::TransformedPointContents(const vec3_t p,
...
====================
*/
void idCollisionModelManagerLocal::AdjustAreaPortalState(sint area1,
        sint area2, bool open) {
    if(area1 < 0 || area2 < 0) {
        return;
    }
    ...
    } else if(cm.areaPortals[area2 * cm.numAreas +
                                   area1]) { // Ridah, fixes loadgame issue
        cm.areaPortals[area1 * cm.numAreas + area2]--;
        cm.areaPortals[area2 * cm.numAreas + area1]--;
...
=================
*/
sint idCollisionModelManagerLocal::WriteAreaBits(uchar8 *buffer,
        sint area) {
