...
sint c_active_windings, c_peak_windings, c_winding_allocs,
     c_winding_points;
...
void FreeWinding(winding_t *w) {
    if(*reinterpret_cast<uint *>(w) == 0xdeaddead) {
        common->Error(ERR_FATAL, "FreeWinding: freed a freed winding");
    }
...
winding_t *BaseWindingForPlane(vec3_t normal, vec_t dist) {
    ...
    if(x == -1) {
        common->Error(ERR_DROP, "BaseWindingForPlane: no axis found");
    }
...
void ClipWindingEpsilon(winding_t *in, vec3_t normal, vec_t dist,
    ...
    sint             sides[MAX_POINTS_ON_WINDING + 4] = { 0 }, counts[3], i, j,
            maxpts;
    float64    dot;
    vec3_t          mid;
    winding_t      *f, *b;
    ...
    *back = b = AllocWinding(maxpts);
    ...
    if(f->numpoints > maxpts || b->numpoints > maxpts) {
        common->Error(ERR_DROP, "ClipWinding: points exceeded estimate");
    }

    if(f->numpoints > MAX_POINTS_ON_WINDING ||
            b->numpoints > MAX_POINTS_ON_WINDING) {
        common->Error(ERR_DROP, "ClipWinding: MAX_POINTS_ON_WINDING");
    }
}
...
void CheckWinding(winding_t *w) {
    ...
    if(w->numpoints < 3) {
        common->Error(ERR_DROP, "CheckWinding: %i points", w->numpoints);
    }

    area = WindingArea(w);

    if(area < 1) {
        common->Error(ERR_DROP, "CheckWinding: %f area", area);
    }
    ...
    for(i = 0; i < w->numpoints; i++) {
        ...
        for(j = 0; j < 3; j++)
            if(p1[j] > MAX_MAP_BOUNDS || p1[j] < -MAX_MAP_BOUNDS) {
                common->Error(ERR_DROP, "CheckFace: MAX_MAP_BOUNDS: %f", p1[j]);
            }
        ...
        if(d < -ON_EPSILON || d > ON_EPSILON) {
            common->Error(ERR_DROP, "CheckWinding: point off plane");
        }
        ...
        if(VectorLength(dir) < ON_EPSILON) {
            common->Error(ERR_DROP, "CheckWinding: degenerate edge");
        }
        ...
        for(j = 0; j < w->numpoints; j++) {
            ...
            if(d > edgedist) {
                common->Error(ERR_DROP, "CheckWinding: non-convex");
            }
        }
    }
}
...
void AddWindingToConvexHull(winding_t *w, winding_t **hull,
    ...
    vec3_t          hullPoints[MAX_HULL_POINTS],
                    newHullPoints[MAX_HULL_POINTS], hullDirs[MAX_HULL_POINTS];
