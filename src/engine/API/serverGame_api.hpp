...
typedef struct svEntity_s {
    ...
    sint
    lastCluster;    // if all the clusters don't fit in clusternums
    sint            areanum, areanum2;
    sint
    snapshotCounter;    // used to prevent double adding from portal views
    sint
    originCluster;  // Gordon: calced upon linking, for origin only bmodel vis checks
} svEntity_t;
...
public:
    ...
    virtual void LocateGameData(sharedEntity_t *gEnts, uint64 numGEntities,
                                sint sizeofGEntity_t, playerState_t *clients, uint64 sizeofGameClient) = 0;
    virtual void GameDropClient(sint clientNum, pointer reason,
                                sint length) = 0;
    virtual void GameSendServerCommand(sint clientNum, pointer text) = 0;
    virtual bool EntityContact(const vec3_t mins, const vec3_t maxs,
                               const sharedEntity_t *gEnt, traceType_t type) = 0;
    ...
    virtual bool GetTag(sint clientNum, sint tagFileNumber, valueType *tagname,
                        orientation_t *_or) = 0;
