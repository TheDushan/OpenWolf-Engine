...
====================
*/
static void SendGetInfo( server_t* server, socket_t recv_socket, bool force_new_challenge )
{
...
====================
*/
static void HandleGetServers( const char* msg, const struct sockaddr_storage* addr, socklen_t addrlen, socket_t recv_socket, bool extended_request )
{
...
====================
*/
int validPacketRange( const char* data, unsigned int min, unsigned int max )
{
...
====================
*/
static void SendResponseToServer( socket_t recvSocket, char challenge[64], const struct sockaddr_storage* address, socklen_t addressLen )
{
    char msg[64] = "ipAuthorize ";
    char info[128];
    
    sprintf( info, "\xFF\xFF\xFF\xFFipAuthorize %s", challenge );
    if( sendto( recvSocket, info, strlen( info ), 0, ( const struct sockaddr* )address, addressLen ) < 0 )
    {
...
====================
*/
static void HandleKeyResponse( const char* msg, const struct sockaddr_storage* addr, socklen_t addrlen, socket_t recv_socket )
{
    char str[128];
    char* token[64] = { 0 };
    ...
    if( !validPacket( token[0], 16 ) || !validPacket( token[1], 5 ) || !validPacket( token[2], 12 ) )
    {
        return;
    }
...
====================
*/
static void HandleIPResponse( const char* message, const struct sockaddr_storage* address, socklen_t addressLen, socket_t recvSocket )
{
    char str[128];
    char* token[64] = { 0 };
    unsigned int index = 0;
...
void HandlePacketMessage( const char* message, size_t length, const struct sockaddr_storage* address, socklen_t addressLen, socket_t recvSocket )
{
    ...
    if( !strncmp( PACKET_CLIENT, message, strlen( PACKET_CLIENT ) ) )
    {
        ...
        HandleKeyResponse( message + strlen( PACKET_CLIENT ), address, addressLen, recvSocket );
    }
    // Server reply
    else if( !strncmp( PACKET_SERVER, message, strlen( PACKET_SERVER ) ) )
    {
        ...
        HandleIPResponse( message + strlen( PACKET_SERVER ), address, addressLen, recvSocket );
    }
}
