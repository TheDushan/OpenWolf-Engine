...
void generic( json_t* json, struct parser_s* mdata )
{
    if( json != NULL && json->child != NULL && json->child->text != NULL )
    {
        ...
        sprintf( sql_data.col_value[sql_data.count].value, "\"%s\"", json->child->text );
        sql_data.count++;
    }
}
...
void genericDate( json_t* json, struct parser_s* mdata )
{
    ...
    {
        strcpy( sql_data.col_value[sql_data.count].col, mdata->sqlname );
        sprintf( sql_data.col_value[sql_data.count].value, "FROM_UNIXTIME(%s)", json->child->text );
        sql_data.count++;
    }
...
void getGameTypeID( json_t* json, struct parser_s* mdata )
{
    ...
    if( json != NULL && json->child != NULL && json->child->text != NULL )
    {
        ...
        if( ( game_type_id = db_get_id_from( buffer ) ) < 0 ) return;
...
void statInsert( json_t* json, struct parser_s* mdata )
{
    ...
    if( json != NULL && json->text != NULL )
    {
        ...
        if( g_game_id == -1 )
        {
            for( i = 0; i < sql_data.count && strcmp( sql_data.col_value[i].col, "gameid" ) != 0; i++ );
            
            if( i != sql_data.count ) gameid = sql_data.col_value[i].value;
            else
            {
...
void viewParserJSON( json_t* json, parser_t* pmenu )
{
    ...
    {
        for( i = 0; pmenu[i].name != NULL && strcmp( pmenu[i].name, json->text ) != 0; i++ );
        
        if( pmenu[i].name != NULL )
        {
            next_menu = pmenu[i].next;
            if( pmenu[i].f != NULL ) pmenu[i].f( json, &( pmenu[i] ) );
        }
        else
        {
            level--;
...
int parse_messageJSON( char* msg )
{
    ...
    while( line != NULL )
    {
        ...
        {
            printf( "Col %s val %s\n", sql_data.col_value[i].col, sql_data.col_value[i].value );
        }
