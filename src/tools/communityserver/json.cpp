...
rcstring *rcs_create(size_t length) {
    rcstring *rcs;
    rcs = (rcstring *)malloc(sizeof(
                                 rcstring));        /* allocates memory for a struct rcstring */
...
rstring_code rcs_resize(rcstring *rcs, size_t length) {
    ...
    temp = (char *)realloc(rcs->text,
                           sizeof(char) * (length + 1));         /* length plus '\0' */
...
rstring_code rcs_catcs(rcstring *pre, const char *pos,
                       const size_t length) {
    assert(pre != NULL);
    assert(pos != NULL);
...
static void intern_json_free_value(json_t **value) {
    ...
    if((*value)->parent) {
        ...
        if((*value)->parent->child == (*value)) {
            if((*value)->next) {
                (*value)->parent->child =
                    (*value)->next;     /* the parent node always points to the first node in the children linked list */
            } else {
        ...
        if((*value)->parent->child_end == (*value)) {
            if((*value)->previous) {
                (*value)->parent->child_end =
                    (*value)->previous;     /* the parent node always points to the last node in the children linked list */
            } else {
...
enum json_error json_insert_pair_into_object(json_t *parent,
        const char *text_label, json_t *value) {
            ...
            case JSON_STRING:
                ...
                if(cursor->parent != NULL) {
                    if(cursor->parent->type ==
                            JSON_OBJECT) {    /* cursor is label in label:value pair */
                ...
                switch(text[++r]) {
                    ...
                    case 'u': {
                        ...
                        if(unicode < 0x80) {
                            ...
                            fprintf(stderr, "JSON: unsupported unicode value: 0x%I64X\n", unicode);
                        }
                    }
                    break;
...
int lexer(const char *buffer, const char **p, unsigned int *state,
          rcstring **text, size_t *line) {
    assert(buffer != NULL);
    assert(p != NULL);
    assert(state != NULL);
    assert(text != NULL);
                ...
                switch(lexer(buffer, &info->p, &info->lex_state, &info->lex_text,
                    ...
                    case LEX_END_OBJECT:
                        if(info->cursor->parent == NULL) {
                            info->state =
                                99;   /* finished document. only accept whitespaces until EOF */
                        } else {
            ...
            case 6: { /* label, pos name separator */
                ...
                switch(value = lexer(buffer, &info->p, &info->lex_state, &info->lex_text,
                    ...
                    case LEX_STRING:
                        ...
                        if(info->cursor->parent == NULL) {
                            info->state =
                                99;   /* finished document. only accepts whitespaces until EOF */
                        } else {
                            info->cursor = info->cursor->parent;
                        }
...
state1:         /* parse string */
    {
        switch(c) {
            case '\\':
                if(!jsps->string_length_limit_reached) {
                    if(rcs_length((jsps->temp)) < JSON_MAX_STRING_LENGTH -
                            1) {        /* check if there is space for a two character escape sequence */
                        if(rcs_catc((jsps->temp), '\\') != RS_OK) {
                            return JSON_MEMORY;
                        }
                    } else {
            ...
            default:
                if(!jsps->string_length_limit_reached) {
                    if(rcs_length((jsps->temp)) <
                            JSON_MAX_STRING_LENGTH) {        /* check if there is space for a two character escape sequence */
                        if(rcs_catc((jsps->temp), c) != RS_OK) {
                            return JSON_MEMORY;
                        }
                    } else {
...
state24:            /* parse number: decimal part */
    {
        switch(c) {
            ...
            case 'E':
                ...
                jsps->string_length_limit_reached =
                    0;  /* reset to accept the exponential part */
                jsps->state = 20;   /* parse number: start exponent part */
                break;
...
json_t *json_find_first_label(const json_t *object,
