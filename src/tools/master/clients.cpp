...
static int Cl_QueryThrottleDecay(client_t *client) {
    int count = client->count - (int)((crt_time - client->last_time) /
                                      fp_decay_time);
...
====================
*/
static qboolean Cl_AddClient(const struct sockaddr_storage *address,
                             socklen_t addrlen) {
    ...
    if(free_client != NULL) {
        ...
        memcpy(&free_client->user.address, address,
               sizeof(free_client->user.address));
...
qboolean Cl_BlockedByThrottle(const struct sockaddr_storage *addr,
    ...
    unsigned int hash;
    client_t *client;
    qboolean(*IsSameAddress)(const struct sockaddr_storage * addr1,
                             const struct sockaddr_storage * addr2, qboolean * same_public_address);
