...
// Description: Games management for dpmaster
// -------------------------------------------------------------------------------------
////////////////////////////////////////////////////////////////////////////////////////
...
cmdline_status_t Game_DeclarePolicy(const char *policy, const char **games,
    ...
    for(i = 0; i < nb_games; i++) {
        ...
        if(! Game_Find(game, &index)) {
            ...
            new_game_names = (const char **)realloc((void *)game_names,
                                                    (nb_game_names + 1) * sizeof(game_names[0]));

            if(new_game_names == NULL) {
                return CMDLINE_STATUS_NOT_ENOUGH_MEMORY;
            }

            memmove((void *)&new_game_names[index + 1], &new_game_names[index],
                    (nb_game_names - index) * sizeof(new_game_names[0]));
...
static game_protocol_assoc_t *game_protocols =
    NULL;    // TODO: sort it to allow binary searches
...
====================
*/
static game_properties_t *Game_GetAnonymous(const char *game,
        qboolean creation_allowed) {
    ...
    for(proto_ind = 0; proto_ind < nb_game_protocols; proto_ind++) {
        if(game_protocols[proto_ind].game != game_props) {
            if(proto_ind != proto_copy_ind) {
                memcpy(&game_protocols[proto_copy_ind], &game_protocols[proto_ind],
                       sizeof(game_protocols[proto_copy_ind]));
            }
...
====================
*/
static cmdline_status_t Game_AddProtocol(game_properties_t *game_props,
        int protocol) {
    unsigned int index;
    ...
    if(! Game_FindAnonymous(protocol, &index)) {
        ...
        new_array = (game_protocol_assoc_t *)realloc(game_protocols,
                    (nb_game_protocols + 1) * sizeof(game_protocols[0]));
        ...
        memmove(&game_protocols[index + 1], &game_protocols[index],
                (nb_game_protocols - index) * sizeof(game_protocols[0]));
...
====================
*/
static cmdline_status_t Game_RemoveProtocol(game_properties_t *game_props,
        int protocol) {
    unsigned int index;
    ...
    if(Game_FindAnonymous(protocol, &index)) {
        ...
        memmove(&game_protocols[index], &game_protocols[index + 1],
                (nb_game_protocols - index - 1) * sizeof(game_protocols[0]));
        nb_game_protocols--;
    }
...
====================
*/
static void Game_RemoveHeartbeat(game_properties_t *game_props,
                                 heartbeat_type_t hb_type) {
...
====================
*/
static cmdline_status_t Game_UpdateHeartbeat(game_properties_t *game_props,
        heartbeat_type_t hb_type, const char *value, qboolean remove) {
    if(remove) {
...
====================
*/
static cmdline_status_t Game_UpdateOption(game_properties_t *game_props,
        const char *option_name, qboolean remove) {
    size_t option_ind;

    for(option_ind = 0; option_strings[option_ind].value != GAME_OPTION_NONE;
            option_ind++) {
        const game_option_string_t *option = &option_strings[option_ind];

        if(strcmp(option->string, option_name) == 0) {
            if(remove) {
                game_props->options = (game_options_t)(game_props->options & ~
                                                       (option->value));
            } else {
                game_props->options = (game_options_t)(game_props->options |
                                                       (option->value));
...
====================
*/
static cmdline_status_t Game_UpdateProperty(game_properties_t *game_props,
    ...
    while(next_value != NULL) {
        ...
        if(strcmp(prop_name, "protocols") == 0) {
            ...
            result = Game_UpdateHeartbeat(game_props, HEARTBEAT_TYPE_ALIVE, next_value,
                                          remove_values);
            ...
            result = Game_UpdateHeartbeat(game_props, HEARTBEAT_TYPE_DEAD, next_value,
                                          remove_values);
...
void Game_InitProperties(void) {
    ...
    size_t game_count = sizeof(builtin_props_array) / sizeof(
                            builtin_props_array[0]);
    size_t game_ind;

    for(game_ind = 0; game_ind < game_count; game_ind++) {
        builtin_props_t *builtin_props = &builtin_props_array[game_ind];
        Game_UpdateProperties(builtin_props->gamename, builtin_props->props,
                              builtin_props->nb_props);
    }
}
...
void Game_PrintProperties(void) {
    ...
    while(props != NULL) {
        ...
        for(ind = 0;
                ind < sizeof(props->heartbeats) / sizeof(props->heartbeats[0]); ind++) {
...
cmdline_status_t Game_UpdateProperties(const char *game,
    ...
    for(prop_ind = 0; prop_ind < nb_props; prop_ind++) {
        ...
        if(equal_sign != NULL && equal_sign != work_buff) {
            ...
            result = Game_UpdateProperty(game_props, work_buff, equal_sign + 1,
                                         reset_property, remove_values);
            free(work_buff);
...
const game_properties_t *Game_GetPropertiesByHeartbeat(
    const char *heartbeat_tag, qboolean *flatline_heartbeat) {
    game_properties_t *props = game_properties_list;
